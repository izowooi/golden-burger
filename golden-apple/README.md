# Golden Apple - Polymarket 자동 매매 봇

확률 기반 Polymarket 자동 매매 봇입니다. 예측 시장에서 80% 이상의 확률에 매수하고, 90% 도달 시 매도하여 안정적인 수익을 목표로 합니다.

## 개요

- **매수 조건**: YES/NO 확률이 80% 이상일 때
- **매도 조건**: 확률이 90% 도달 시
- **목표 수익**: 거래당 ~10%
- **리스크 관리**: 손절 없음, 한 번 거래한 시장 재거래 금지
...
stateDiagram-v2
    ...
    Scanning --> BuyCandidate: 80% ≤ 확률 < 90%
    Scanning --> Skip: 확률 < 80% or ≥ 90%
    ...
    SellCheck --> Holding: 확률 < 90%
    SellCheck --> Sell: 확률 ≥ 90%
...
```bash
git clone https://github.com/your-repo/golden-apple.git
cd golden-apple
```
...
`close_only`는 보유분을 즉시 전량 매도하지 않습니다. Apple의 기존 매도 조건인 확률 90%
이상일 때만 SELL을 시도하므로 끝까지 남는 포지션은 별도 퇴역 플레이북에 따라 처리합니다.
또한 모드 전환 전에 접수된 GTC BUY 주문은 자동 취소되지 않으므로 저장소 루트의
...
| 확률 80% 이상 도달 | 매수 |
| 확률 90% 이상 도달 | 매도 |
| 80% 건너뛰고 바로 90% 이상 | 거래 안함 (급등) |
| 매수 후 확률 하락 | 보유 유지 (손절 없음) |
| 90% 달성 못하고 만료 | 보유 유지 |
| 이미 거래한 시장 | 재거래 금지 |
