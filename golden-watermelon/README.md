...
Active v3c data contract는 `soccer-inplay-elite-competition-match-winner-v3`다. 축구
...
|---|---|---:|
| `polybot-white` | `watermelon-white-1m-v3c` | `FAST_1M`, `* * * * *` |
...
검증한다. child moneyline, extra time, penalty shoot-out 또는 advancement market은 제외한다.

volume/liquidity 하한은 없다. 각 eligible outcome의 full-depth CLOB ask/bid를 저장해 `$5`부터
`$500`까지 사후 replay할 수 있게 한다. displayed depth는 guaranteed fill이 아니다.
...
full book은 frozen notional ladder `$5/$10/$15/$20/$25/$30/$40/$50/$75/$100/$150/$250/$500`를
...
uv run polybot config --simulate --job watermelon-grey-5m-v3c
```
...
[PREREGISTRATION.md](research/frozen-2026-08-26-uefa-clock-scale-v3c/PREREGISTRATION.md)를
따른다.

## Immutable epochs

v3c는 기존 DB를 migration, `ALTER TABLE`, copy, merge, backfill, delete 또는 clean하지 않고
...
과거 data contract `soccer-inplay-major-league-match-winner-v2`와
