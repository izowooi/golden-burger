...
Active v3a data contract: `soccer-inplay-major-league-match-winner-v2`.
...
`polybot-white`의 `watermelon-white-1m-v3a`는 1분(`FAST_1M`), `polybot-grey`의
`watermelon-grey-5m-v3a`는 5분(`CONTROL_5M`) cadence로
동일한 전체 모집단을 관측한다. 두 DB의 같은 `condition × token × threshold`는 paired
cadence 실험이며 두 거래로 세지 않는다.

v3a 모집단은 축구만 포함한다. EPL(`epl`), Bundesliga(`bun`), Ligue 1(`fl1`),
LaLiga(`lal`), MLS(`mls`)의 Gamma sport id/code/exact name/primaryTagId/series id+slug,
...
uv run polybot config --simulate --job watermelon-grey-5m-v3a
```
...
[PREREGISTRATION.md](research/frozen-2026-08-24-soccer-v3a/PREREGISTRATION.md)를 따른다.

## v3a evidence와 immutable legacy epochs
...
`watermelon-*-v2`, `watermelon-white-1m-v3`, `watermelon-grey-5m-v3` 및 그
`soccer-inplay-major-league-match-winner-v1` DB는 immutable archive evidence다. v3a가 이를
