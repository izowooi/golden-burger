...
def _classify_direct_sport_event(
    ...
    identity: DirectSportIdentity | None = gamma.direct_identities_by_code.get(
        sport_family
    )
    if identity is None:
        raise ValueError(f"unsupported direct sport family: {sport_family}")
    ...
    teams = (
        ...
        if isinstance(event.get("teams"), list)
        else []
    )
    ...
    if not reasons:
        status = "ACCEPTED"
    elif any(reason.endswith("EXCLUDED") for reason in reasons):
        status = "REJECTED"
    else:
        status = "DRIFT"
...
def _cup_identity_mismatch_reasons(
    ...
    *,
    ...
    reasons: list[str] = []
    required_tags = {*common_tag_ids, str(identity.tag_id)}
    if not required_tags <= set(tag_ids):
        reasons.append("EVENT_REQUIRED_TAG_IDS_MISSING")
    if series_ids != (identity.series_id,):
        reasons.append("EVENT_SERIES_ID_MISMATCH")
    if series_slugs != (identity.series_slug,):
    ...
    if not event_slug.startswith(identity.event_slug_prefix):
        reasons.append("EVENT_COMPETITION_SLUG_PREFIX_MISMATCH")
    resolution_source = str(event.get("resolutionSource") or "").strip()
    source_host = (urlsplit(resolution_source).hostname or "").casefold()
...
def classify_soccer_event(event: Mapping[str, Any], gamma: GammaConfig) -> LeagueClassification:
    ...
    cup_candidates = [
        identity for identity in gamma.cup_mapping
...
def classify_sports_event(
    ...
    family = str(sport_family or "").strip().casefold()
    if family == "soccer":
        result = classify_soccer_event(event, gamma)
        result.evidence.setdefault("sport_family", "soccer")
        return result
    if family in gamma.direct_identities_by_code:
        return _classify_direct_sport_event(event, gamma, family)
    raise ValueError(f"unsupported sport family: {family}")
