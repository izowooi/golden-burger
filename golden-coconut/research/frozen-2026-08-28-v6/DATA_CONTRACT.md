# Golden Coconut v6 Data Contract

Data contract: `major-sports-lifecycle-census-v6`. Daily-rsync collection contract:
`research-full-v1`.

- Accountless `archive_only`; simulation/shadow modes only.
- Canonical artifact: `data/coconut-major-sports-lifecycle-5m-v6/trades_sim.db`.
- UTC archives are immutable whole files named `trades_sim_YYYYMMDD.db`.
- SQLite is standalone create-only schema v6 and append-only. It never opens, migrates, merges,
  copies, or backfills a v1/v2/v3/v4/v5 database.
...
- Every public HTTP attempt has independent connect/read timeouts and a 15-second total streamed
  response boundary. Partial bytes and `AttemptWallTimeout` are durable request evidence; exhausted
  retries make the family/cycle fail rather than silently dropping a page.
...
Analyzer input is restricted to daily-rsync-verified v6 shards from one cohort and to atomically
