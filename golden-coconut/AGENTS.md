...
1. `research/EPOCHS.json`의 active epoch와
   `research/frozen-2026-08-28-v7/PREREGISTRATION.md`, `DATA_CONTRACT.md`, `MANIFEST.sha256`
2. v7이 hash로 그대로 재사용하는
...
- family별 logical Gamma `/events/keyset` sweep은 `closed=false`, 실제 경기 시작 시각
  ...
  다섯 family는 서로 다른 public HTTP session의 worker 다섯 개로 동시에 시작하고, 결과는 frozen
  family order로만 정규화한다. worker 하나라도 실패하면 partial census를 승인하지 않는다.
...
`data/coconut-major-sports-lifecycle-5m-v7/trades_sim.db`지만 SQLite table에는 `orders`, `fills`,
