...
Golden Coconut은 계좌·주문 경로가 없는 major-sports in-play moneyline research collector다.
축구(EPL, Bundesliga, Ligue 1, LaLiga, MLS, Serie A, UCL, UEL), MLB, NBA, NFL, NHL을
동일한 5분 cadence에서 관측한다. canonical runtime은 `coconut-major-sports-5m-v1`, 예정
Jenkins job은 `polybot-gold`다.
...
각 cycle은 다음 5개 Gamma `/events/keyset` sweep을 독립적으로 terminal cursor까지 읽는다.
...
모든 sweep은 `closed=false`, `live=true`, `related_tags=false`다. liquidity와 `volumeNum`,
...
`$5/$10/$25/$50/$100/$250/$500` executable ladder를 walk한다. fee endpoint가 응답한 값만
...
uv run polybot config --simulate --job coconut-major-sports-5m-v1
uv run polybot status --simulate --job coconut-major-sports-5m-v1
uv run polybot health --simulate --job coconut-major-sports-5m-v1
uv run polybot analyze --simulate --job coconut-major-sports-5m-v1 \
  --db /absolute/path/to/trades_sim.db
```

Analyzer는 health, sport-equal coverage, season-separated liquidity/volume/depth/threshold strata,
game clustering과 missing-sport null을 보고한다. health-only 자료에서 profitability 결론은 항상
`null`이다. 운영 절차는 [OPERATIONS.md](OPERATIONS.md), frozen 질문은
