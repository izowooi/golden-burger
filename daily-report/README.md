...
Polymarket Data API 조회
  ...
  → Supabase RPC에서 카탈로그 전체 계정+합계를 한 transaction으로 확정
...
정상 통합 리포트는 사람이 읽는 fallback text를 사용하고, `pb-portfolio/v3` marker와
`COMPLETE`, `tz=<IANA timezone>`은 summary footer에만 기록합니다. v3는 계정별 `author_name` + 첫 text 줄의
`$total (Position: $position, Cash: $cash)` 형식이며 **Jenkins 설정과 Supabase 카탈로그의 전체 계정**만 허용합니다.
...
중복 이름 처리 후의 표시 이름 집합이 Supabase 카탈로그와 정확히 일치하는지 검증한다.
하나라도 누락·미등록되면 데이터
...
연결 점검이 성공하면 `Supabase 연결/계정 계약 확인 성공 - 계정 카탈로그: <N>개`가 출력됩니다. 이 명령은 계정 mapping과 atomic writer RPC contract를 읽기 전용으로 확인하며 Slack을 보내거나 DB를 수정하지 않습니다.
