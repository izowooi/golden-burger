...
아홉 개 Polymarket 계정의 잔고를 조회해 Slack으로 보고하고, 같은 일일 스냅샷을 Supabase의 `pb_*` 테이블에 저장하는 Jenkins 작업입니다.
...
Polymarket Data API 조회
  ...
  → Supabase RPC에서 9계정+합계를 한 transaction으로 확정
...
정상 통합 리포트는 `pb-portfolio/v3` marker와 `COMPLETE`, `tz=<IANA timezone>`을
fallback text와 summary footer에 기록합니다. v3는 계정별 `author_name` + 첫 text 줄의
`$total (Position: $position, Cash: $cash)` 형식이며 **현재 9계정 전체**만 허용합니다.
...
`pb_write_complete_portfolio_snapshot_v3`는 선택 사항이 아닙니다. writer는 아홉 계정
...
| `polymarket-golden-lion-address` | golden-lion 테스트 슬롯 주소 |
| `polymarket-golden-tiger-address` | golden-tiger 테스트 슬롯 주소 |
...
중복 이름 처리 후의 9개 표시 이름뿐 아니라 각 표시 이름의 stable `account_id`가
...
연결 점검이 성공하면 `Supabase 연결/계정 계약 확인 성공 - 계정 카탈로그: 9개`가 출력됩니다. 이 명령은 계정 mapping과 atomic writer RPC contract를 읽기 전용으로 확인하며 Slack을 보내거나 DB를 수정하지 않습니다.
...
# 키/DB 읽기 권한과 9계정 env-catalog 일치 확인 (Slack/DB 수정 없음)
