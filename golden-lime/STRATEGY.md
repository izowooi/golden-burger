...
**운영 결과: 14일간 계정 -72%** ($239.89 → $66.36). 그리고 `market_snapshots` **103만 행**으로
사후 백테스트를 수행한 결과, **가설을 지지하는 증거가 없었다.**

| 검정 | 결과 |
|---|---|
| barrier 승률 (운영 설정) | **41.2%** — 무편향 가격의 이론값 **40.0%** 와 사실상 동일 |
| 파라미터 격자 28개 조합 | 전부 **\|t\| < 1.3**. 어느 방향으로도 유의한 표류 없음 |
| 유일한 양수 셀 | z=1.96이나 Bonferroni(필요 \|z\|>3.1) 미통과. 기간 이분할 시 전반 65.7% → 후반 57.1%로 감쇠 |
| 반대 가설(elderberry 방향) | 함께 기각. 급락 후 12h 평균 **-18.92%** (t=-2.10) — 반등이 아니라 계속 하락 |
| 손실의 정체 | 시뮬레이션 예측 손실이 실제 계좌 손실을 **98% 정확도로 설명**. 잃은 것은 **거래비용 그 자체** |

즉 위에 서술한 "거래량·고점 유지로 구분되는 하위 집단은 지속한다"는 조건부 예외는
**존재하지 않았다.** 이 문서가 스스로 인용한 리서치("10%+ 급변은 mean-revert")가 맞았다.

구현 결함(신호가 구간 천장을 감지, 수동적 지정가로 모멘텀 편승, 트레일링이 승자 절단)도
모두 확인되어 수정했으나, **그것을 다 고쳐도 부호가 바뀌지 않는다.** 청산 구조의 기대값은
왕복 거래비용의 음수(EV = −c)이고 이는 파라미터로 바꿀 수 없다.

**→ 상세: [docs/retro/golden-lime-2026-07-backtest-verdict.md](../docs/retro/golden-lime-2026-07-backtest-verdict.md)**
