...
class TradeRepository:
    ...
    def get_holding_trade(self, condition_id: str) -> Optional[Trade]:
        """해당 시장의 HOLDING 포지션 조회."""
    ...
    def can_enter(
        ...
        """재진입 가능 여부 판정 (영구 one-shot 대신 쿨다운 방식).

        - HOLDING 포지션이 있으면 불가
        - 마지막 COMPLETED sell_timestamp가 쿨다운 이내면 불가
        - 마지막 skipped_at이 쿨다운 이내면 불가
        - 그 외에는 재진입 허용
        ...
        if self.get_holding_trade(condition_id):
        ...
        if last_sell is not None and last_sell >= cutoff:
            return False, f"cooldown_sell({last_sell.isoformat()})"
        ...
        if last_skip is not None and last_skip >= cutoff:
            return False, f"cooldown_skip({last_skip.isoformat()})"
    ...
    def mark_as_skipped(self, condition_id: str, reason: str) -> SkippedMarket:
        """Mark a market as skipped (timestamp 기반 쿨다운, 영구 밴 아님)."""
    ...
    def get_snapshots_since(
        ...
        """특정 마켓의 timestamp 기반 스냅샷 조회 (오래된 것 먼저).

        개수(limit) 기반이 아니라 timestamp 기반으로 조회한다 -
        Jenkins 중단 시 윈도우가 왜곡되는 banana 버그 방지.
        """
    ...
    def cleanup_old_snapshots(self, days: int = 7) -> int:
        ...
        Returns:
            삭제된 스냅샷 수
        """
        if compact_maintenance_active(self.session, "golden-lime"):
            return 0
    ...
    def append_trade_to_csv(self, trade: Trade, db_dir) -> None:
        ...
        row = {
            ...
            "jump_size_at_buy": trade.jump_size_at_buy or "",
            "base_price_at_buy": trade.base_price_at_buy or "",
