...
def _validate_config(trading: TradingConfig, api: ApiConfig) -> None:
    ...
    numeric = {
        ...
        "shock.jump_window_hours": shock.jump_window_hours,
        "shock.jump_min": shock.jump_min,
        "shock.base_min": shock.base_min,
        "shock.base_max": shock.base_max,
        "shock.current_max": shock.current_max,
        "shock.hold_window_minutes": shock.hold_window_minutes,
        "shock.max_pullback": shock.max_pullback,
        "shock.vol_mult_min": shock.vol_mult_min,
        "shock.death_window_hours": shock.death_window_hours,
    }
    ...
    if shock.jump_window_hours <= 0 or shock.hold_window_minutes <= 0 or shock.death_window_hours <= 0:
    ...
    if shock.base_min + shock.jump_min > shock.current_max:
        raise ValueError("shock.base_min + jump_min must not exceed current_max")
    if shock.hold_window_minutes > shock.jump_window_hours * 60:
        raise ValueError("shock.hold_window_minutes must fit inside jump_window_hours")
    if not 0 <= shock.max_pullback < shock.jump_min:
        raise ValueError("shock.max_pullback must be >= 0 and < jump_min")
    if shock.vol_mult_min < 1:
        raise ValueError("shock.vol_mult_min must be >= 1")
