...
3. timer 없이 `polybot-do → polybot-re → polybot-mi`를 한 번씩 실행한다.
4. 각 build에서 cursor-complete Gamma, 두 token book, SQLite quick_check, status/health를 확인한다.
5. timer를 각각 0/1/2 minute offset으로 추가한다.
6. 최소 2회 자연 실행 뒤 duplicate/off-slot/overlap이 없는지 확인한다.
