...
class GammaClient:
    ...
    def get_market_by_condition_id(
        ...
        try:
            # Gamma's default /markets view can omit resolved rows.  Query the
            # current view first, then the explicit closed view.  limit=2 lets
            # us reject duplicate identities rather than silently choosing one.
