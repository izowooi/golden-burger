...
def _reason_key(reason: str) -> str:
    ...
    예: dev_below_min_+0.012 → dev_below_min, price_out_of_band_0.28 → price_out_of_band
    """
...
class MarketScanner:
    ...
    def __init__(
        ...
        self._cycle_window_points: Dict[str, List[SnapshotPoint]] = {}
    ...
    def save_market_snapshots(self, markets: List[Dict]) -> int:
        """스냅샷과 complete-sweep membership을 한 transaction으로 저장한다.
            ...
            for market in markets:
                ...
                if not outcome_prices or len(outcome_prices) < 2:
                    snapshot_results[condition_id] = {
                        "snapshot_eligible": False,
                        "snapshotted": False,
                        "snapshot_reason": "missing_outcome_prices",
                    }
                    continue
                ...
                try:
                    ...
                    snapshot_results[condition_id] = {
                        "snapshot_eligible": False,
                        "snapshotted": False,
                        "snapshot_reason": "invalid_yes_price",
                    }
                    continue
    ...
    def _build_window_points(
        ...
        if condition_id in self._cycle_window_points:
            points = self._cycle_window_points.pop(condition_id)
        else:
    ...
    def _prefetch_missing_histories(
        ...
        """Batch only histories whose locally archived 24h window is invalid."""
        self._cycle_window_points = {}
        if (
            not self.repo
        ...
        for market in markets:
            ...
            points = to_points(self.repo.get_snapshots_since(condition_id, since))
            self._cycle_window_points[condition_id] = points
