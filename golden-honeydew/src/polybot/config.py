...
def _validate_config(trading: TradingConfig, api: ApiConfig) -> None:
    ...
    numeric = {
        ...
        "quiet.start_hour": quiet.start_hour,
        "quiet.end_hour": quiet.end_hour,
        "signal.median_lookback_hours": signal.median_lookback_hours,
        "signal.dev_min": signal.dev_min,
        "signal.vol_spike_block": signal.vol_spike_block,
        "signal.entry_prob_min": signal.entry_prob_min,
        "signal.entry_prob_max": signal.entry_prob_max,
        "time_based.entry_hours_min": timing.entry_hours_min,
        "time_based.exit_hours": timing.exit_hours,
        "time_based.max_holding_hours": timing.max_holding_hours,
    }
    ...
    if not 0 <= quiet.start_hour < 24 or not 0 <= quiet.end_hour < 24 or quiet.start_hour == quiet.end_hour:
        raise ValueError("quiet hours must be distinct UTC hours between 0 and 23")
    if signal.median_lookback_hours <= 3:
        raise ValueError("signal.median_lookback_hours must be > the 3h recent-volume window")
    if not 0 < signal.dev_min < 1:
        raise ValueError("signal.dev_min must be between 0 and 1")
    if signal.vol_spike_block < 1:
        raise ValueError("signal.vol_spike_block must be >= 1")
    if not 0 < signal.entry_prob_min < signal.entry_prob_max < 1:
        raise ValueError("signal probability bounds must satisfy 0 < entry_prob_min < entry_prob_max < 1")
    if not 0 < timing.exit_hours <= timing.entry_hours_min or timing.max_holding_hours <= 0:
        raise ValueError("time_based hours must satisfy 0 < exit_hours <= entry_hours_min and max_holding_hours > 0")
