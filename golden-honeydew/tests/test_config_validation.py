...
def test_signal_probability_bounds_must_be_ordered(monkeypatch):
    monkeypatch.setenv("POLYBOT_ENTRY_PROB_MIN", "0.9")
    monkeypatch.setenv("POLYBOT_ENTRY_PROB_MAX", "0.3")
    with pytest.raises(ValueError, match="probability bounds"):
        load_config("missing.yaml")


def test_invalid_boolean_env_is_rejected(monkeypatch):
    monkeypatch.setenv("POLYBOT_QUIET_WEEKENDS", "tru")
