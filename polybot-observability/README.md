...
여러 `golden-*` 전략이 공통으로 사용하는 로컬 관측성 패키지다. 주문 가격이나 전략 판단을
...
Queen, Papaya, Blueberry, Kiwi, Melon, Quince는 새 runtime DB에 한해 `prepare_database(...,
...
`prune_daily_logs()`는 이 여섯 전략의 `YYYYMMDD.log`만 60일 기준으로 정리한다. 앱 로그와
별개인 Jenkins console log 보존은 Jenkins Build Discarder에서 관리한다.
