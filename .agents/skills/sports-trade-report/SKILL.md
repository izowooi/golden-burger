...
축구 → MLB → NFL → NBA/NHL 순으로 모든 경기 행을 나열한다. 본문 표의 위나 아래에 **전략/Jenkins별 종목 통합 합계와 전체 합계**를 자동 계산해 표시한다. 각 종목의 경기×전략 표에는 전략 열별 합계와 경기 행별 합계, 종목 전체 합계를 함께 넣어 사용자가 수동으로 더하지 않게 한다.

합계는 같은 UTC 기간·같은 본문 경기 범위에서 원래 Decimal 금액을 먼저 더하고 마지막에 표시 반올림한다. 실제 매도 순손익, 정산 지급권 손익, 확인 부분 합계를 별도 열/행으로 보인다. 미수집·미체결·수수료 미확정·미연결 주문·잔여 노출은 0으로 채우지 않는다. 확인 부분 합계 옆에 미확정 건수를 유지하며, 과거 잔여 노출 부록·다른 기간·simulation을 본문 합계에 섞지 않는다. 원장/경기별 JSON 합계와 종목별·통합 합계가 맞는지 검증한다.

각 경기 안에서 strategy/Jenkins/runtime/팔·선택 팀 또는 YES/NO·BUY/SELL UTC/KST·confirmed 수량/VWAP/원금·exit/stop/resolution·순손익 또는 unknown 사유를 적는다. 진입 당시 policy와 원장 청산 사유를 체결 행과 함께 표시해 왜 사고팔았는지 설명하며, 기록이 없으면 미확인으로 남긴다. 최신 API가 응답했다는 이유만으로 체결·수익을 정상이라고 쓰지 않는다.
