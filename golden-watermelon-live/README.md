...
두 arm의 유일한 family 내 treatment는 진입 하한이다. cadence는 모두 1분이다.
Cat/Dog Soccer와 NFL의 신규 목표 주문액은 각각 독립된 `$10`이며 MLB child는 close-only `$5`다.
Soccer `.91/.92`는 40경기 탐색 뒤 continuous live A/B로 배포한 하한이며 아직 최적값으로 확정하지 않는다.
