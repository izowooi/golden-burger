...
def test_timed_out_buy_quarantine_is_event_local_but_keeps_capacity(tmp_path):
    db_path = tmp_path / "plum-stale-buy-quarantine.db"
    Session = init_database(str(db_path))
    ExecutionLedger(db_path, strategy_name="golden-plum")
    session = Session()
    repo = TradeRepository(session)
    trade = repo.create_trade(
...
def test_reconciled_positive_orphan_buy_is_atomically_recovered(tmp_path):
    ...
    session = Session()
    observed = datetime(2026, 8, 25, 12, 0, 0)
    snapshot_start = MarketSnapshot(
    ...
    snapshot_middle = MarketSnapshot(
        condition_id="condition-recover",
        event_id="event-recover",
        token_id="token-recover",
        outcome="Yes",
    ...
    session.add_all([snapshot_start, snapshot_middle, snapshot])
    session.flush()
    session.add(
        MarketCatalog(
            condition_id="condition-recover",
    ...
    session.add(
        EntryEpisode(
            ...
            in_play_hours=1.0,
            source_elapsed_minutes=20,
            trend_start_snapshot_id=snapshot_start.id,
            trend_middle_snapshot_id=snapshot_middle.id,
    ...
    session.execute(
        text(
            "UPDATE order_submissions SET latest_order_status='MATCHED', "
            f"latest_size_matched={requested_size}, making_amount=5.0, "
            "needs_reconciliation=0 "
    ...
    session.execute(
        text(
            ...
            f"'CONFIRMED', 'BUY', {requested_size}, 0.75, 0.005, "
            "'2026-08-25T00:00:00Z', NULL)"
        ),
    ...
    assert trade.buy_shares == pytest.approx(requested_size)
    assert trade.buy_confirmed_fee_usdc == pytest.approx(0.005)
    assert trade.outcome_side == "YES"
    assert trade.result_kind == "HOME"
    assert trade.take_profit_delta_at_buy == pytest.approx(0.15)
    assert trade.take_profit_price_at_buy == pytest.approx(0.90)
    assert trade.stop_loss_delta_at_buy == pytest.approx(0.15)
