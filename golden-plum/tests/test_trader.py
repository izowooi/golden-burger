...
class _Repo:
    ...
    def get_recent_token_snapshots(self, _token_id, *, limit):
        assert limit == 3
        return [
            SimpleNamespace(
                id=9,
                ...
                source_elapsed_minutes=18,
            ),
            SimpleNamespace(
                id=10,
                probability=0.74,
                timestamp=(NOW - timedelta(minutes=1)).replace(tzinfo=None),
                source_elapsed_minutes=19,
            ),
            SimpleNamespace(
                id=11,
                probability=0.75,
                timestamp=NOW.replace(tzinfo=None),
                source_elapsed_minutes=20,
            ),
        ]
...
def _candidate():
    return {
        ...
        "entry_episode_id": 3,
        "trend_start_snapshot_id": 9,
        "trend_middle_snapshot_id": 10,
        "trend_snapshot_ids": [9, 10, 11],
        "trend_prices": [0.72, 0.74, 0.75],
        "trend_cumulative_move": 0.03,
        "trend_max_pullback": 0.0,
        "trend_elapsed_seconds": 120.0,
        "yes_probability": 0.75,
        "game_start_time": NOW - timedelta(minutes=20),
...
def test_buy_revalidates_exact_five_and_submits_fok(monkeypatch) -> None:
    ...
    assert created["buy_price"] == 0.75
    assert created["buy_shares"] == pytest.approx(5 / 0.75)
    assert created["status"] is TradeStatus.PENDING_BUY
    assert created["yes_price_at_buy"] == 0.75
    assert created["stop_price_at_entry"] == pytest.approx(0.60)
...
def test_confirmed_buy_freezes_entry_relative_stop_from_actual_vwap() -> None:
    ...
    assert update["stop_price_at_entry"] == pytest.approx(0.84)
...
def test_entry_relative_stop_uses_frozen_fifteen_point_reversal() -> None:
    ...
    # The frozen Plum stop is fifteen percentage points below confirmed entry.
    trade = SimpleNamespace(
        id=9,
        condition_id="condition-1",
        event_id="event-1",
    ...
    assert Trader.effective_stop_price(trade, TradingConfig()) == pytest.approx(0.84)
    assert trader.execute_sell(trade) is False
...
def test_continuous_stop_failure_is_quarantined_after_three_hours(
    ...
    monkeypatch.setattr(trader_module, "datetime", _FixedDatetime)
    repo, clob = _Repo(), _Clob(best_bid=0.83, best_ask=0.84)
    trade = SimpleNamespace(
        id=91,
...
def test_recovered_stop_clears_continuous_failure_timer(monkeypatch) -> None:
    monkeypatch.setattr(trader_module, "datetime", _FixedDatetime)
    repo, clob = _Repo(), _Clob(best_bid=0.89, best_ask=0.895)
    trade = SimpleNamespace(
        id=94,
        status=TradeStatus.HOLDING,
        condition_id="condition-1",
...
def test_minute_eighty_forces_exit_and_preserves_stop() -> None:
    ...
    assert len(losing_clob.orders) == 1
    assert losing_repo.updated[-1][1]["exit_reason"] == (
