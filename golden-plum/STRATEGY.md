...
- 최고 호가만 선택하지 않고, 가격 구간 안에서 가장 낮은 VWAP 결과를 선택한다.
  동률은 token ID로 고정하고 주문 직전 같은 선택을 다시 확인한다.
- 현재 호가 1회면 충분하다. 상승폭0.02·되돌림0.01·연속3/5회·이전값의 하한 돌파는 없다.
...
- Silver/Gold는 1분 수집 시작 시 전체 archive 정리·vacuum을 하지 않는다. 별도 유지보수로
  미루며 원자료를 삭제하지 않는다. event 상태 갱신은 현재 run 인덱스로 범위를 제한한다.

현재 소스의 값은 초기 비교 가정이며 수익성·확증편향 인과를 입증한 값이 아니다.
구성/소스 해시가 달라지므로 과거 성과와 분리한다. Jenkins의 예전 ENTRY/TREND/TP
환경변수는 배포 때 제거해야 하며, 코드 push만으로 고정 workspace가 바뀌지는 않는다.

## 2026-09-05 운영자 손실 방어 선택

Queen MLB는 명시적인 사용자 승인에 따라 `POLYBOT_DRAWDOWN_GUARD_ENABLED=false`로
누적 손실에 의한 신규 매수 중지를 해제할 수 있다. 기본값은 true이며 이 예외는
`plum-live-queen-mlb-95-1m-v1`에만 허용한다. Queen 축구·King·다른 종목에는 전파하지 않는다.
...
## 2026-09-06 실행 증거 보완 — 운영 배포 전

- BUY의 가격 개선으로 요청보다 많은 주식을 받는 경우, 요청 수량의 105%라는 임의
  상한으로 거절하지 않는다. 같은 주문의 거래소 매칭 수량, 실제 체결 가격, 저장된
  주문 원금 한도와 누적 체결액을 함께 검증한다. SELL 수량은 제출 수량을 넘길 수 없다.
  human/fixed-6 단위를 검증하고, 수량 검증 실패와 수수료 조회 실패를 별도로 기록한다.
- 새 FOK SELL은 실제 서명 객체에서 계산한 주문 해시와 계좌 지문·토큰·수량을 POST 전에
  같은 DB에 보존한다. 개인 키와 서명 원문은 저장하지 않는다. 응답 유실 시 cycle당
  최대 한 건의 정확한 주문 해시를 조회하고, 계좌·토큰·방향·수량·가격이 일치할 때만
  기존 intent에 주문번호를 연결한다. 이후 체결·수수료 확인은 기존 절차를 그대로 따른다.
- 조회 실패, 없는 주문 응답, 응답 해시 불일치는 미체결 증거가 아니다. 중복 POST,
  임의 종결, 체결 행 생성 또는 수동 보유 편입을 하지 않는다. 원래 서명 주문 정보가
  없는 과거 SELL에는 이 복구 방법을 소급 적용할 수 없다.
- 이 보완은 진입·익절·손절 파라미터를 바꾸지 않는다. 로컬 모의 응답과 SDK 검증의
  통과는 실제 거래소 체결 확인 또는 운영 배포 완료를 뜻하지 않는다. 운영 DB에 남은
  과거 미확정 주문은 별도 실제 주문·체결 증거로 확인해야 한다.
