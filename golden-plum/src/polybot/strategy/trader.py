"""Order execution and evidence-safe settlement for Golden Plum."""
...
def _orphan_episode_contract_matches(
    episode: object,
    config: TradingConfig,
) -> bool:
    """Require the complete frozen trend lineage before orphan recovery."""
    try:
        snapshot_ids = (
            int(getattr(episode, "trend_start_snapshot_id")),
            int(getattr(episode, "trend_middle_snapshot_id")),
            int(getattr(episode, "entry_snapshot_id")),
        )
        observations = int(getattr(episode, "trend_observations"))
    ...
    return bool(
        ...
        and all(snapshot_id > 0 for snapshot_id in snapshot_ids)
...
class Trader:
    """Trade only bot-owned direct result tokens under the Plum contract."""
    ...
    def execute_buy(self, candidate: dict) -> Optional[int]:
        ...
        if normalized_episode_id is None:
            return self._reject_entry("entry_episode_missing")
        execution_trend, trend_reason = evaluate_trend_confirmation(
            self.repo.get_recent_token_snapshots(
                token_id,
                limit=self.config.entry.trend_observations,
            ),
            current_snapshot_id=entry_snapshot_id,
            config=self.config,
        )
        reported_ids = tuple(candidate.get("trend_snapshot_ids") or ())
        reported_prices = tuple(candidate.get("trend_prices") or ())
        if execution_trend is None:
            return self._reject_entry(f"execution_{trend_reason}")
        if (
            reported_ids != execution_trend.snapshot_ids
            or len(reported_prices) != len(execution_trend.prices)
            or any(
                not math.isclose(float(reported), expected, rel_tol=0, abs_tol=1e-9)
                for reported, expected in zip(
                    reported_prices, execution_trend.prices, strict=True
                )
            )
        ...
        trade = self.repo.create_trade(
            ...
            take_profit_delta_at_buy=(
                self.config.entry.take_profit_price - float(walk.vwap)
            ),
            ...
            trend_elapsed_seconds=execution_trend.elapsed_seconds,
    ...
    def recover_orphan_buys(self) -> dict:
        ...
        for submission in submissions:
            ...
            lineage_contract_ok = _orphan_episode_contract_matches(
                episode, self.config
            )
            lineage = []
            lineage_trend = None
            if lineage_contract_ok:
                lineage_ids = [
                ...
                lineage_trend, _lineage_reason = evaluate_trend_confirmation(
                    lineage,
                    current_snapshot_id=int(episode.entry_snapshot_id),
                    config=self.config,
                )
            ...
            try:
                ...
                submitted_price = float(submission.get("requested_price"))
            except (TypeError, ValueError):
                signed_maker_usdc = math.nan
                submitted_size = math.nan
                submitted_price = math.nan
            if (
                snapshot is None
                or catalog is None
                ...
                or not (
                    ...
                    and float(episode.exact_vwap) - 1e-9
                    <= submitted_price
                    <= self.config.entry.prob_max + 1e-9
                )
            ...
            trade = self.repo.create_recovered_orphan_trade(
                ...
                trend_cumulative_move=episode.trend_cumulative_move,
                trend_max_pullback=episode.trend_max_pullback,
                trend_elapsed_seconds=episode.trend_elapsed_seconds,
