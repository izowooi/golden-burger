...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Polymarket Automated Trading Bot (Cascade Rider)",
...
Examples:
  ...
  polybot config
    ...
    # Setup logging: --verbose는 DEBUG로 최우선, 아니면 LOG_LEVEL env(기본 INFO)
    ...
    if args.command == "run":
        ...
        cascade = config.trading.cascade
        print("=== Bot Configuration ===")
        print(f"Job Name: {config.job_name}")
        print(f"Simulation Mode: {config.simulation_mode}")
        print(f"DB Path: {config.db_path}")
        print()
        print("=== Cascade Rider Strategy ===")
        print(f"Price Band: {cascade.prob_min:.0%} ~ {cascade.prob_max:.0%}")
        print(f"Drift Band ({cascade.drift_lookback_hours}h): "
              f"{cascade.drift_min:+.0%} ~ {cascade.drift_max:+.0%}")
        print(f"Bucket Hours: {cascade.bucket_hours}h "
              f"({cascade.drift_lookback_hours // cascade.bucket_hours} buckets)")
        print(f"Consistency Min: {cascade.consistency_min:.0%}")
        print(f"Volume Accel Min: {cascade.vol_accel_min:.1f}x")
        print(f"Drift Death Window: {cascade.death_window_hours}h")
        print()
        ...
        print(f"Take Profit: {config.trading.take_profit_percent:+.0%} (목표가 0.99 캡)")
        print(f"Stop Loss: {config.trading.stop_loss_percent:+.0%}")
        print(f"Trailing Stop: {config.trading.trailing_stop.percent:.0%} "
              f"(enabled: {config.trading.trailing_stop.enabled})")
        print(f"Entry Hours Min: {config.trading.entry_hours_min}h")
        print(f"Exit Hours: {config.trading.exit_hours}h")
