...
- `scripts/analyze_shadow.py`: Shadow cohort/2×2 uniqueness를 fail-closed로 검사하는 analyzer
...
- capacity에는 `QUARANTINED`와 trade 미연결/unknown live BUY intent도 포함하며 GTC BUY TTL은 10분이다.
...
uv run python scripts/analyze_experiment.py --arm-a <a-db> --arm-b <b-db> --review-start <RFC3339-UTC> --review-end <exclusive-RFC3339-UTC> --output-dir <artifact-dir>
uv run python scripts/analyze_shadow.py --db <shadow-db> --review-start <RFC3339-UTC> --review-end <exclusive-RFC3339-UTC> --output-dir <artifact-dir>
```
...
- Shadow analyzer는 missing cohort나 duplicate exact treatment row를 dedup하지 않고 summary를 거부한다.
...
- DB별 sibling nonblocking run lock을 유지하고 Gamma pre-order 100 page/225초 cap을 cycle 전체 timeout으로 확장하지 않는다.
