"""Repository operations for Closing Surge trades and research evidence."""
...
class TradeRepository:
    ...
    def record_entry_signal_decision(
        ...
        """Persist one first-crossing A/B decision without creating cooldown."""
    ...
    def cleanup_old_snapshots(self, days: int = 60) -> int:
        ...
        self.session.execute(
            text(
                ...
                "_polybot_blueberry_protected_snapshots "
                "(id INTEGER PRIMARY KEY) WITHOUT ROWID"
            )
        )
        self.session.execute(text("DELETE FROM _polybot_blueberry_protected_snapshots"))
        self.session.execute(
            text(
