"""SQLite evidence models for Golden Blueberry / Closing Surge."""
...
class Trade(Base):
    """One submitted Closing Surge position and its observed evidence."""
    ...
    snapshot_gap_minutes_at_entry = Column(Float)
    surge_at_entry = Column(Float)
    min_surge_at_entry = Column(Float)
...
class MarketSnapshot(Base):
    """Closing Surge research observation, always expressed as a YES price."""
...
class EntrySignalDecision(Base):
    """One durable record for each first observed threshold crossing.

    Rejected crossings are as important as submitted orders for the A/B
    post-mortem. This table records the treatment value and downstream metadata
    gates without turning the rejection into a re-entry cooldown.
    """
...
_TRADE_MIGRATION_COLUMNS = {
    ...
    "snapshot_gap_minutes_at_entry": "REAL",
    "surge_at_entry": "REAL",
    "min_surge_at_entry": "REAL",
...
def init_database(
    ...
    prepare_database(db_path, "golden-blueberry", requirements=maintenance_requirements)
