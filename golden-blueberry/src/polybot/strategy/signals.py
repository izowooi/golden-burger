"""Pure signal functions for the Closing Surge strategy."""
...
def evaluate_entry(
    ...
    *,
    ...
    """Evaluate one Closing Surge entry.
    ...
    min_surge = _param(params, "min_surge")

    if previous is None:
        return EntryDecision(False, "no_prior_snapshot", None, current, hours, None)
    if current is None:
        return EntryDecision(False, "invalid_current_price", previous, None, hours, None)
    surge = current - previous
    normalized_phase = str(phase or "").strip().lower()
    if normalized_phase not in {"scheduled", "pregame", "in_play"}:
        return EntryDecision(
            False, "invalid_entry_phase", previous, current, hours, surge
        )
    if normalized_phase != "in_play":
        if hours is None or not math.isfinite(hours):
            return EntryDecision(
                False, "no_entry_deadline", previous, current, None, surge
            )
        if hours < 0:
            return EntryDecision(
                False, "entry_deadline_passed", previous, current, hours, surge
            )
        if hours <= 0:
            return EntryDecision(
                False, f"too_late_{hours:.1f}h", previous, current, hours, surge
            )
        ...
        if hours > hours_max + EPSILON:
            return EntryDecision(
                False, f"too_early_{hours:.1f}h", previous, current, hours, surge
            )
    ...
    if surge < min_surge - EPSILON:
        return EntryDecision(
            False,
            f"surge_below_min_{surge:.3f}",
            previous,
            current,
            hours,
            surge,
    ...
    return EntryDecision(
        True,
        (
            f"closing_surge_{previous:.3f}_to_{current:.3f}_"
            f"delta_{surge:.3f}_in_play"
            if normalized_phase == "in_play"
            else f"closing_surge_{previous:.3f}_to_{current:.3f}_"
            f"delta_{surge:.3f}_{hours:.1f}h"
        ),
        previous,
        current,
        hours,
