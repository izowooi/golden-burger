"""Gamma universe archive and threshold-crossing scanner for Closing Surge."""
...
from .timing import EntryClock, evaluate_entry_clock, parse_market_datetime


logger = logging.getLogger(__name__)
_BOOK_TOLERANCE = 1e-6
_NUMERIC_REASON_PART = re.compile(r"^[+-]?\d[\d.]*[a-z%]*$")
_BLUEBERRY_ARCHIVE_MIN_LIQUIDITY = 1_000.0
...
class MarketScanner:
    ...
    def __init__(
        ...
        self.config = config
        self.repo = repo
        # Kept in the signature for shared construction code; Closing Surge relies
    ...
    def _archive_decision(
        ...
        yes_price: float,
        now: datetime,
    ) -> tuple[bool, str, EntryClock]:
        clock = evaluate_entry_clock(market, self.config.sports, now)
        strict_reason = strict_binary_reason(market)
        if strict_reason != "ok":
            return False, strict_reason, clock
        if is_excluded_market(market, self.config.excluded_categories):
            return False, "excluded_category", clock
        if not clock.valid:
            return False, clock.reason, clock
        if (
            clock.phase != "in_play"
            and clock.hours_left is not None
            and clock.hours_left > self.config.archive.hours_max + 1e-9
        ):
            return False, f"archive_too_early_{clock.hours_left:.1f}h", clock
        if yes_price < self.config.archive.prob_min - 1e-9:
            return False, f"archive_price_below_{yes_price:.3f}", clock
        return True, "archive_eligible", clock
    ...
    def save_market_snapshots(
        ...
        if self.repo is None:
            raise RuntimeError("repository is required for Closing Surge archive evidence")
        ...
        try:
            for market in markets:
                ...
                eligible, reason, _ = self._archive_decision(
            ...
            raise
        logger.info(
            "Closing Surge research snapshot %s개 저장 (strict binary, YES>=%.2f, <=%.0fh)",
            saved,
    ...
    def _entry_snapshot_lineage(
        ...
        # Query persisted history rather than trusting process-local caches.
        # Sixty-day retention is much wider than Blueberry's three-day archive.
    ...
    def scan_buy_candidates(
        ...
        """Return markets that have just crossed into the Closing Surge band."""
        reference = now or datetime.now(timezone.utc)
        if reference.tzinfo is None:
            reference = reference.replace(tzinfo=timezone.utc)
        ...
        for market in markets:
            ...
            resolution_hours = get_hours_until_resolution(end_date, reference)
            ...
            # Persist only an actual first crossing. Below-threshold inventory
            # is archived but would create millions of low-value decision rows.
            crossed = (
            ...
            def record_signal(result: str, reason: str) -> None:
                ...
                self.repo.record_entry_signal_decision(
                    ...
                    surge=float(yes["probability"]) - float(prior.probability),
                    snapshot_gap_minutes=gap_minutes,
                    min_surge=self.config.entry.min_surge,
                    ...
                    effective_min_liquidity=(
                        self.config.effective_min_liquidity
                    ),
            ...
            candidate = {
                ...
                "end_date": end_date,
                "hours_until_resolution": resolution_hours,
                ...
                "game_start_time": clock.game_start_time,
                "minutes_until_game_start": clock.minutes_until_game_start,
