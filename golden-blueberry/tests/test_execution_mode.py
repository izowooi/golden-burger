"""Shared tick-rounding invariants and Blueberry's fixed execution mode.

Blueberry does not test maker/taker execution.  Both A/B arms are locked to
``nearest`` and differ only in the minimum consecutive surge.  The passive and
cross assertions remain here because the shared CLOB wrapper supports them;
the config-validation tests prove that Blueberry cannot select those modes.
"""
...
class TestControlArms:
    def test_nearest_matches_legacy_fleet_behaviour(self):
        """Both Blueberry arms retain the fleet's neutral rounding behavior."""
