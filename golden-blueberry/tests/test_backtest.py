"""Deterministic, offline Blueberry research backtest contract."""
...
SPEC = importlib.util.spec_from_file_location("blueberry_backtest", SCRIPT)
...
def research_csv(tmp_path):
    ...
    rows = [
        ...
        _row("train-stop", "2026-01-01T00:00:00Z", 20, 0.84),
        ...
        _row("test-resolve", "2026-02-01T00:00:00Z", 10, 0.84),
        ...
        _row("test-loss", "2026-03-01T00:00:00Z", 6, 0.84, resolution_payout=0),
        _row(
            "test-loss",
...
def test_declared_grid_is_complete():
    ...
    assert len(params) == 2
    assert {item.entry_probability for item in params} == {0.85}
    assert {item.entry_price_max for item in params} == {0.93}
    assert {item.min_surge for item in params} == {0.02, 0.05}
    assert {item.stop_probability for item in params} == {0.78}
    assert {item.take_profit_probability for item in params} == {0.97}
    assert {item.min_liquidity for item in params} == {10_000}
    assert {item.min_volume_24h for item in params} == {10_000}
    assert {item.entry_hours_max for item in params} == {72}
...
def test_run_writes_hashed_csv_json_manifest_and_no_database(
    ...
    assert manifest["engine_version"] == "blueberry-offline-v1"
