"""Closing Surge scanner crossing and event-risk identity contracts."""
...
from polybot.config import SurgeEntryConfig, TradingConfig
...
def test_first_crossing_below_arm_surge_is_durably_recorded_as_rejected(caplog):
    repo = PriorRepo(probability=0.849)
    repo.current.probability = 0.86
    config = TradingConfig(entry=SurgeEntryConfig(min_surge=0.05))
    scanner = scanner_with_lineage(repo=repo, config=config)
    ...
    assert "surge_below_min: 1" in caplog.text
    evidence = repo.signal_decisions[0]
    assert evidence["decision"] == "rejected"
    assert evidence["reason"] == "surge_below_min"
    assert evidence["surge"] == pytest.approx(0.011)
    assert evidence["min_surge"] == pytest.approx(0.05)
...
def test_stale_prior_snapshot_fails_closed_at_explicit_gap(caplog):
    scanner = scanner_with_lineage(repo=PriorRepo(gap_minutes=15.01))
...
def test_exact_max_snapshot_gap_is_inclusive():
    scanner = scanner_with_lineage(repo=PriorRepo(gap_minutes=15))
