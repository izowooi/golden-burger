"""Golden Blueberry config precedence, defaults, and DB isolation."""
...
def test_closing_surge_defaults_are_fail_closed():
    config = load_config("missing.yaml", job_name="blueberry")
    trading = config.trading

    assert trading.lifecycle_mode == "active"
    assert trading.buy_amount_usdc == 5.0
    assert trading.min_liquidity == 10_000.0
    assert trading.min_volume_24h == 10_000.0
    ...
    assert trading.max_spread == 0.02
    assert trading.depth_price_window == 0.01
    assert trading.depth_safety_multiple == 1.20
    assert trading.yes_only_mode is True
    assert trading.excluded_categories == []
    # Closing Surge 고유 계약: 실행 모드는 처치축이 아니라 기존 봇과 같은
    # 중립값 nearest이며, 낙폭 kill switch가 코드에 존재한다.
    assert trading.execution_mode == "nearest"
    assert trading.intent_autoresolve is True
    assert trading.max_drawdown_stop == 0.20
    assert trading.experiment_capital_usdc == 150.0
    assert trading.effective_min_liquidity == 10_000.0
    assert trading.effective_min_volume_24h == 10_000.0
    assert trading.ab_arm == "A-2pp"
    assert len(trading.strategy_source_digest) == 64
    assert trading.entry.prob_min == 0.85
    assert trading.entry.prob_max == 0.93
    assert trading.entry.stop_price == 0.78
    assert trading.entry.take_profit_price == 0.97
    ...
    assert trading.sports.reject_without_game_start is False
    assert trading.sports.max_in_play_minutes == 360.0
...
def test_all_strategy_env_names_resolve(monkeypatch):
    values = {
        ...
        "POLYBOT_TAKE_PROFIT_PRICE": "0.985",
        ...
        "POLYBOT_ALLOW_IN_PLAY": "false",
        "POLYBOT_MAX_IN_PLAY_MINUTES": "240",
    ...
    assert trading.buy_amount_usdc == 5
    ...
    assert trading.entry.take_profit_price == 0.985
    ...
    assert trading.sports.allow_in_play is False
    assert trading.sports.max_in_play_minutes == 240
...
def test_private_key_prefix_is_stripped_and_missing_credentials_fail(monkeypatch):
    ...
    assert load_config("missing.yaml", simulation_mode=True).api.private_key == ""
    with pytest.raises(ValueError, match="POLYMARKET_PRIVATE_KEY"):
        load_config("missing.yaml", simulation_mode=False)
...
def test_initial_live_size_is_hard_capped_but_reviewed_scale_is_automatic(monkeypatch):
    from polybot.config import TradingConfig

    trading = load_config("missing.yaml").trading
    assert trading.effective_min_liquidity == 10_000
    assert trading.effective_min_volume_24h == 10_000
    assert trading.max_open_notional_usdc == 50

    monkeypatch.setenv("POLYBOT_BUY_AMOUNT", "100")
    with pytest.raises(ValueError, match="hard cap"):
        load_config("missing.yaml")

    promoted = TradingConfig(buy_amount_usdc=100, max_buy_amount_usdc=100)
    assert promoted.effective_min_liquidity == 200_000
    assert promoted.effective_min_volume_24h == 200_000
    assert promoted.max_open_notional_usdc == 1_000
