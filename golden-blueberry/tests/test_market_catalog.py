"""Closing Surge replay catalog, broad archive, and atomic sweep evidence."""
...
def test_fresh_database_starts_with_compact_storage(tmp_path):
    database = tmp_path / "fresh-blueberry.db"
    ...
    assert (active, strategy) == (1, "golden-blueberry")
    assert report["requirements"]["full_cadence_hours"] == 0.25
...
def activate_compact_maintenance(session, *, active=1):
    ...
    report = json.dumps(
        {
            "policy": asdict(policy_for("golden-blueberry", requirements)),
    ...
    session.execute(
        text(
            ...
            "last_report_json) "
            "VALUES ('compact-v1', 2, 'golden-blueberry', :active, :activated_at, "
            ":last_report_json)"
        ),
        {
            "active": active,
...
def test_complete_sweep_catalogs_all_qualified_but_snapshots_only_archive_set(
    ...
    raw = [
        market("archive-floor", yes_price=0.75, hours_left=168),
        market("below-floor", yes_price=0.749),
        market("neg-risk", yes_price=0.90, neg_risk=True),
        market("not-binary", yes_price=0.90, outcomes=("Up", "Down")),
        market("too-early", yes_price=0.90, hours_left=169),
        market(
            "invalid-book",
            yes_price=0.90,
            best_bid=0.91,
    ...
    assert snapshot.probability == 0.75
    assert snapshot.liquidity == 200_000
    assert snapshot.volume_24h == 200_000
    ...
    assert memberships["below-floor"].snapshot_reason == "archive_price_below_0.749"
...
def test_tampered_sweep_digest_rolls_back_catalog_snapshot_and_sweep(tmp_path):
    session, repo, gamma, scanner = build_scanner(
        tmp_path, [market("archive-floor", yes_price=0.75)]
    )
    qualified = scanner.fetch_markets()
...
def test_rejected_first_crossing_is_durable_and_later_recross_fails_closed(
    ...
    below = market("durable", yes_price=0.84, hours_left=12)
    snapshot_cycle(gamma, repo, config, below, NOW)

    first_crossing = market("durable", yes_price=0.86, hours_left=12)
...
def test_sub_two_hour_first_crossing_is_an_entry_candidate(tmp_path):
    ...
    below = market("sports-window", yes_price=0.84, hours_left=1.5)
    snapshot_cycle(gamma, repo, config, below, NOW)
...
def test_window_rejected_first_crossing_is_durable_after_entry_window_opens(
    ...
    # Keep one fixed resolution time. The first crossing is outside Blueberry's
    # 72-hour entry window, but remains inside the 168-hour archive envelope.
    below = market("window", yes_price=0.84, hours_left=120)
    snapshot_cycle(gamma, repo, config, below, NOW)
    first_crossing = market("window", yes_price=0.90, hours_left=100)
    first_scanner, first_markets = snapshot_cycle(
        gamma,
        repo,
        config,
    ...
    recross = market("window", yes_price=0.90, hours_left=60)
    recross_scanner, recross_markets = snapshot_cycle(
        gamma,
        repo,
        config,
...
def test_fresh_ask_rejected_first_crossing_is_durable_and_not_retried(
    ...
    below = market("ask", yes_price=0.84, hours_left=12)
    snapshot_cycle(gamma, repo, config, below, NOW)
    ...
    dip = market("ask", yes_price=0.84, hours_left=12)
    snapshot_cycle(gamma, repo, config, dip, NOW + timedelta(minutes=10))
