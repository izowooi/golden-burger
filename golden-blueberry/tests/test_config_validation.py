"""Strict cross-field validation for Closing Surge configuration."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("POLYBOT_BUY_AMOUNT", "5.01", "hard cap"),
        ("POLYBOT_MAX_BUY_AMOUNT_USDC", "100", "not an environment override"),
        ("POLYBOT_MIN_SURGE", "0.03", "preregistered A/B"),
        ("POLYBOT_EXECUTION_MODE", "passive", "requires.*nearest"),
        ...
        ("POLYBOT_MAX_SPREAD", "0", "max_spread"),
        ("POLYBOT_DEPTH_SAFETY_MULTIPLE", "0.99", "depth_safety_multiple"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"POLYBOT_ENTRY_HOURS_MAX": "120.01"},
            "hours_max",
        ),
        (
...
def test_entry_and_archive_boundaries_are_accepted(monkeypatch):
    ...
    monkeypatch.setenv("POLYBOT_TAKE_PROFIT_PRICE", "0.99")
...
def test_yaml_cannot_raise_code_hard_cap(tmp_path):
    path = tmp_path / "bad.yaml"
    path.write_text(
        "trading:\n  max_buy_amount_usdc: 100\n",
        encoding="utf-8",
    )
    with pytest.raises(ValueError, match="code hard cap"):
        load_config(str(path))
