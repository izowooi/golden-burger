#!/usr/bin/env python3
"""Offline, replayable research backtest for Golden Blueberry.
...
ENGINE_VERSION = "blueberry-offline-v1"
ENTRY_GRID = (0.85,)
ENTRY_UPPER_GRID = (0.93,)
MIN_SURGE_GRID = (0.02, 0.05)
STOP_GRID = (0.78,)
TAKE_PROFIT_GRID = (0.97,)
ORDER_NOTIONAL_USDC = 5.0
MIN_LIQUIDITY_FLOOR = 10_000.0
MAX_ORDER_LIQUIDITY_RATIO = 0.0005
MIN_VOLUME_24H_FLOOR = 10_000.0
    ...
    for index, current in enumerate(observations[1:], start=1):
        ...
        if crossed:
            ...
            executable = (
                ...
                and current.yes_price - previous.yes_price >= params.min_surge
            )
            in_window = (
    ...
    for entry in ENTRY_GRID:
        for entry_upper in ENTRY_UPPER_GRID:
            if entry_upper < entry:
                continue
            for min_surge in MIN_SURGE_GRID:
...
def write_artifacts(
    *,
    ...
    manifest = {
        ...
        "assumptions": {
            "entry": "first yes_price crossing plus consecutive minimum surge; hypothetical fill at observed best_ask",
