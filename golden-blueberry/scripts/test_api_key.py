...
def test_api_connection() -> bool:
    ...
    signature_type_raw = os.getenv("POLYMARKET_SIGNATURE_TYPE", "1")

    signature_type_valid = True
    try:
        signature_type = int(signature_type_raw)
        if signature_type not in {1, 3}:
            raise ValueError
    except ValueError:
        print_result(False, "POLYMARKET_SIGNATURE_TYPE must be 1 or 3")
        all_passed = False
        signature_type_valid = False
        signature_type = 1
    ...
    if not private_key:
        ...
        print_result(True, "Private key present")
    ...
    if not funder_address:
        ...
        print_result(True, "Funder address present")

    if signature_type_valid:
        print_result(True, f"Signature Type: {signature_type}")
    ...
    try:
        ...
        print_result(True, "API key derived")
    ...
    if all_passed:
        ...
        print()
        print("You can now run a safe simulation cycle:")
        print("  uv run polybot run --simulate --job blueberry-sim-a-2pp")
        print("Live trading requires explicit --live and a dedicated account/job/DB.")
    else:
