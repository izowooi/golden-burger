...
- 선택 전략 및 전략별 시작 잔고, 종료 잔고, 손익, 단순 수익률 비교
...
상단 KPI는 현재 선택한 전략 계정별로 선택 기간 안의 첫 관측과 마지막 관측을 계산한 뒤 시작 잔고, 종료 잔고, 손익을 각각 합산합니다. 따라서 신규 계정의 관측 이력이 짧아도 장기 운영 계정의 30D·90D 성과가 7D로 축소되지 않습니다. 해당 기간에 관측이 없는 계정은 합산에서 제외하고 실제 포함 계정 수를 표시합니다. 전략별 카드에는 각 계정의 실제 관측 구간과 관측 수를 별도로 표시합니다.

## 데이터 품질 표시

- 최신 보고 시각은 일일 전체 total의 `reported_at` 최신값을 우선하며, total이 없을 때만 계정별 최신 시각으로 대체합니다.
- API 조회 시각과 최신 보고 시각의 차이가 36시간을 넘으면 `리포트 지연`으로 표시합니다.
- 계정별 결측일은 그 계정의 첫 관측일부터 전체 최신 보고일까지 calendar day 단위로 계산합니다.
- 날짜별 전체 total과 같은 날짜의 계정 `total_value` 합계 차이가 $0.01을 초과하면 날짜와 차액을 표시합니다.
- 결측 calendar day를 차트 데이터에 명시적으로 넣고 선을 연결하지 않으므로 수집 공백을 정상 추이처럼 보이지 않습니다.

이 검사는 이상을 가시화할 뿐 자동 보정하지 않습니다. 합계 불일치나 결측이 보이면 Jenkins 적재 로그와 Supabase 원본 행을 확인해야 합니다.
