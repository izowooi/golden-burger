...
상단 KPI는 선택한 계정의 실제 잔고 행을 날짜별로 합산한 뒤, 선택 기간 안에서 실제 행이 있는 가장 이른 날짜의 합계와 가장 늦은 날짜의 합계를 비교합니다. 각 날짜에는 그날 실제로 존재하는 선택 계정 행만 포함하며, 없는 계정 값을 다른 날짜에서 가져오거나 보간하지 않습니다. 전략별 카드에는 각 계정의 실제 관측 구간과 관측 수를 별도로 표시합니다.
