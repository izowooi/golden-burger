...
상단 KPI는 현재 선택한 전략 계정만 합산합니다. 계정별 운용 시작일이 다를 때 신규 계정 투입을 기간 수익으로 잘못 포함하지 않도록, 선택 계정 모두의 데이터가 존재하는 **공통 관측일**만 시작점과 종료점 후보로 사용합니다. 전략별 카드에는 각 계정의 실제 관측 구간과 관측 수를 별도로 표시합니다.
