...
class ClobClientWrapper:
    ...
    def __init__(
        ...
        *,
        ...
        if not simulation_mode:
            raise RuntimeError(
                "Golden Kiwi is research/simulation-only; live CLOB client "
    ...
    def _ensure_initialized(self):
        ...
        if not self.simulation_mode:
            raise RuntimeError(
                "Golden Kiwi is research/simulation-only; authenticated CLOB "
                "initialization is disabled by source policy"
            )
        ...
        try:
            ...
            if self.simulation_mode:
                # Public L0 client: simulation must not require, derive, or
                # transmit account credentials merely to read books/prices.
                self._client = ClobClient(
                    host=self.HOST,
                    chain_id=self.config.chain_id,
                )
                self._initialized = True
                logger.info("CLOB public read client 초기화 완료 (Kiwi simulation)")
                return
    ...
    def place_market_buy(
        ...
        raise RuntimeError(
            "Golden Kiwi live order placement is disabled by source policy"
        )

        try:  # pragma: no cover - retained only for a future reviewed promotion
    ...
    def place_limit_order(
        ...
        # Kiwi is a research-only top-of-book counterfactual. Preserve the
        # exact observed executable price; inherited live tick rounding would
        # make the trade row and simulated submission ledger disagree.
        try:
            simulated_price = float(price)
            simulated_size = float(size)
        except (TypeError, ValueError) as error:
            raise ValueError("simulation price and size must be numeric") from error
        if (
            not math.isfinite(simulated_price)
            or not 0 < simulated_price < 1
            or not math.isfinite(simulated_size)
            or simulated_size <= 0
        ):
            raise ValueError("simulation price/size is outside the valid range")

        if self.simulation_mode:
            logger.info(
                "[SIM] Limit %s - %.6f주 @ %.6f, token: %s",
                side,
                simulated_size,
                simulated_price,
                token_id,
    ...
    def get_open_orders(self) -> list:
        ...
        if not self.simulation_mode:
            raise RuntimeError(
                "Golden Kiwi is research/simulation-only; authenticated CLOB "
                "order reads are disabled by source policy"
            )
        try:
    ...
    def cancel_order(self, order_id: str) -> Dict[str, Any]:
        ...
        if not self.simulation_mode:
            raise RuntimeError(
                "Golden Kiwi is research/simulation-only; live CLOB order "
                "cancellation is disabled by source policy"
            )
    ...
    def test_connection(self) -> bool:
        ...
        if not self.simulation_mode:
            raise RuntimeError(
                "Golden Kiwi is research/simulation-only; authenticated CLOB "
                "connection tests are disabled by source policy"
            )
        try:
            self._ensure_initialized()
