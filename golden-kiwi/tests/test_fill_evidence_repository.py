...
SQLite database as the Kiwi repository.  Accepted order intent alone must
...
def test_exact_fill_evidence_reads_real_ledger_states(tmp_path):
    db_path = tmp_path / "kiwi-ledger.db"
    Session = init_database(str(db_path))
    ledger = ExecutionLedger(db_path, strategy_name="golden-kiwi")
...
def test_pending_sell_completes_from_real_buy_and_sell_ledger_rows(tmp_path):
    ...
    db_path = tmp_path / "kiwi-pending-sell.db"
    Session = init_database(str(db_path))
    ledger = ExecutionLedger(db_path, strategy_name="golden-kiwi")
    ...
    # Live construction is intentionally impossible for Kiwi.  Exercise the
    # inherited exact-fill transition directly so its future-promotion
    # evidence contract cannot silently rot.
    trader = Trader.__new__(Trader)
    trader.repo = repo
    trader.clob = SimpleNamespace(simulation_mode=False)
