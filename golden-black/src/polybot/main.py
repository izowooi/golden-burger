"""Golden Black command line interface."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--live" in arguments:
        print("Golden Black is accountless simulation-only; --live is forbidden", file=sys.stderr)
        return 2
    ...
    if args.command == "run":
        ...
        logging.info("Golden Black cycle completed: %s", json.dumps(result, sort_keys=True))
    ...
    if args.command == "analyze":
        result = analyze_database(args.db or config.db_path)
