"""Bounded server-filtered Gamma event discovery."""
...
class GammaClient:
    ...
    def fetch_sports_events(self, run_id: str, *, observed_at: datetime) -> EventSweep:
        ...
        for page_number in range(1, self.config.max_pages + 1):
            params: dict[str, Any] = {
                ...
                "closed": "false",
                "tag_slug": "sports",
                "liquidity_min": self.config.min_liquidity,
                "volume_min": self.config.min_total_volume,
                "end_date_min": self._iso(observed_at),
                "end_date_max": self._iso(observed_at + timedelta(hours=self.config.end_window_hours)),
            }
            if after_cursor:
            ...
            response = self.transport.request_json(
                ...
                request_kind="gamma_sports_events_keyset",
                run_id=run_id,
