...
**`polybot-date` job은 2026-07-29 중단됐다.** 아래는 그 근거다.

**가설 검정 결과.** 14일간 계정 **-52.2%**($4,730.91 → $2,260.61).
체결 733건의 실제 해결 결과 대조에서 **edge -1.56pp (z=-1.08)** — 양수 edge의 증거가
없다. 손실의 정체는 **작은 음수 edge × 회전율 14.8배**이며, `max_positions: -1`이
그 회전을 만들었다.

초판은 "청산 기계가 건강하다(손절이 $628 절약)"고 적었으나 **2026-07-29 철회한다**
— 체결 기록 결손 artifact였고, 올바르게 재계산하면 +$324 (p=0.65)로 0과 구별되지 않는다. 즉 "망가진 전략"이 아니라 "edge가 없는데
과도하게 굴린 전략"이다. 그래도 최선의 파라미터 조합으로 도달 가능한 것이 **본전**이라 폐쇄했다.

**결정적으로, 이 전략은 아래 §5 "3. 판단 기준"에 스스로 사전 등록한 시험에 이미
실패했다**: 승률 `>= 70%` 요구에 **실측 35.7%**, 평균 손익 `> 0` 요구에 음수.
거래 수(582건)는 기준(30건)을 19배 충족하므로 표본 부족이 아니다. 또한 §5 "2.
소액 실전"이 정한 `POLYBOT_BUY_AMOUNT=5` 2주 관측을 건너뛰고 **첫날부터 $100(20배)**
로 실행됐다.

**살릴 수 있는 진입 레버는 실측으로 전부 기각됐다**: 스포츠 제외(비스포츠 edge
**-2.5pp**로 오히려 더 나쁨), 모멘텀 상한(효과가 단조롭지 않음 = 잡음), 밴드 조정
(최악 칸 순위가 표본을 자르는 방식에 따라 뒤집힘). 양수 edge를 가진 부분표본이
존재하지 않는다. 상세는 판정 문서 §7.

→ [판정 문서](../docs/retro/golden-date-2026-07-verdict.md)

아래는 폐쇄 판정 이전에 작성된 원래 설계 문서다. 역사 자료로만 읽을 것.
