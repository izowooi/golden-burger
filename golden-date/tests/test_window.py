...
def snaps_at_minutes_ago(minutes_list, price=0.85):
...
class TestMergeSnapshots:
    def test_merges_and_sorts(self):
        db = snaps_at_minutes_ago([300, 100], price=0.85)
        backfill = snaps_at_minutes_ago([200, 50], price=0.84)
    ...
    def test_dedupes_same_minute_primary_wins(self):
        db = snaps_at_minutes_ago([100], price=0.85)
        backfill = snaps_at_minutes_ago([100], price=0.70)  # 같은 시각
        merged = merge_snapshots(db, backfill)
        assert len(merged) == 1
        assert merged[0].probability == 0.85  # DB(primary) 우선
