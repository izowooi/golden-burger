"""Golden Queen config precedence, defaults, and DB isolation."""
...
def test_crown_cross_defaults_are_fail_closed():
    config = load_config("missing.yaml", job_name="queen")
    trading = config.trading

    assert trading.lifecycle_mode == "active"
    assert trading.buy_amount_usdc == 100.0
    assert trading.min_liquidity == 5_000.0
    assert trading.min_volume_24h == 1_000.0
    ...
    assert trading.effective_min_liquidity == 25_000.0
    ...
    assert trading.entry.take_profit_price == 0.98
    assert trading.entry.hours_min == 0.0
    assert trading.entry.hours_max == 24.0
    assert trading.archive.prob_min == 0.80
    assert trading.archive.hours_max == 72.0
...
@pytest.mark.parametrize(
    ...
    [
        ("5", 5_000.0, 1_000.0, 50.0),
        ("100", 25_000.0, 2_000.0, 1_000.0),
        ("200", 50_000.0, 4_000.0, 2_000.0),
        ("400", 100_000.0, 8_000.0, 4_000.0),
        ("1000", 250_000.0, 20_000.0, 10_000.0),
    ],
)
