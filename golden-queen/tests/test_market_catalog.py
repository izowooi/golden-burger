...
def build_scanner(tmp_path, markets, *, archive_floor=0.80):
    Session = init_database(str(tmp_path / "queen-evidence.db"))
    session = Session()
...
def activate_compact_maintenance(session, *, active=1):
    ...
    report = json.dumps(
        {
            "policy": asdict(policy_for("golden-queen", requirements)),
    ...
    session.execute(
        text(
            ...
            "last_report_json) "
            "VALUES ('compact-v1', 2, 'golden-queen', :active, :activated_at, "
            ":last_report_json)"
        ),
        {
            "active": active,
...
def test_existing_market_sweeps_table_gets_membership_detail_migration(tmp_path):
    db_path = tmp_path / "legacy-queen.db"
