...
> | **진입 임계값** | **보류** (`drop_min`, `current_max`, `min_liquidity`) | EXPIRED 실제 해결 편입 시 +12.88pp이나 t=+1.64로 검정력 부족. §6-c |
>
> **§6에 최종 권고 config를 통째로 적었다.** 안정성 파라미터는 답이 확정됐고
> (`max_positions: 20` 적용 확인), 청산 배리어는 "바꾸지 마라"가 검정 결과다.
>
> **2026-08-04 후속(§6-c): EXPIRED의 실제 해결 결과를 조회해 생존편향을 실측했다.**
> H1 우위는 +14.74pp → **+12.88pp(t=+1.64)** 로 줄지만 사라지지 않는다. 전략 전체는
> **−0.57%** 로 여전히 0 근처다. H1은 **기각이 아니라 검정력 부족에 의한 보류**다.
...
## 6-c. 【2026-08-04】 EXPIRED 해결 결과 실측 — H1은 보류 유지

> ### ⚠️ 이 절의 초판(2026-08-03)은 틀렸다
>
> 초판은 "Polymarket UI에서 redeem 가능한 것이 1건뿐 → EXPIRED는 대부분 전손"이라고
> 추론해 **H1을 기각**하고 전략을 **폐쇄 후보**로 표시했다.
>
> **그 추론은 무효다.** 운영자 계정은 **auto-redeem이 켜져 있다.** 이긴 포지션은
> 자동으로 USDC가 되어 redeem 목록에서 사라지므로, "목록에 1건뿐"은 승패에 대해
> 아무것도 말해주지 않는다. 추론 대신 **해결 결과를 Gamma에서 직접 조회**해 정정한다.
...
| EXPIRED 총계 | 57건 (리뷰 시점 36건) | 운영 DB |
| 그 중 CONFIRMED BUY fill 존재 | 23건 (40%) | 운영 DB |
| 유령 포지션 정리 | 43 → 8 (35건 종결) | `reconcile_positions.py` |

### EXPIRED의 실제 해결 결과 (Gamma 직접 조회)

로컬 스냅샷(2026-07-28)의 EXPIRED 36건 중 CONFIRMED BUY가 있는 **18건**을
`gamma-api.polymarket.com/markets`에서 `outcomePrices`로 판정했다. 공개 API이며 키가 필요 없다.

| 판정 | n | 비고 |
|---|---:|---|
| **WIN** | **7** | 매수한 outcome의 최종가 ≥ 0.99 |
| **LOSS** | **6** | 최종가 ≤ 0.01 |
| 미확정 | 5 | Gamma가 `closed=true`인데 가격이 0.5 — UMA 미해결/void 추정 |

```
전손 비율 = 6/13 = 46.2%      (초판 추정치 96%는 틀렸다)
판정 13건: 투입 $1,094.87 → 회수 $1,189.47 = +8.6%
```

**EXPIRED는 전손 바구니가 아니라 오히려 수익 구간이었다.**

### H1 재검정 — 축소되지만 사라지지 않는다

| 모집단 | H1 통과 | H1 탈락 | 차이 | t | 전략 전체 |
|---|---:|---:|---:|---:|---:|
| EXPIRED 제외 (§3-1) | +9.63% (n=33) | −5.11% (n=68) | +14.74pp | +3.71 | −0.29% |
| **EXPIRED 실제 해결 편입** | **+7.91%** (n=39) | **−4.98%** (n=75) | **+12.88pp** | **+1.64** | **−0.57%** |
| *(참고) 전손 가정 — 초판의 오판* | −7.24% | −13.97% | +6.73pp | +0.85 | −11.66% |

EXPIRED 판정분 13건만 보면:

| | n | 승 | 투입 → 회수 |
|---|---:|---:|---|
| H1 통과 | 6 | 3/6 | $508 → $590 = **+16.2%** |
| H1 탈락 | 7 | 4/7 | $587 → $599 = +2.1% |

**H1이 고른 바구니가 EXPIRED 안에서도 더 나았다.** 초판이 걱정한
"필터가 관측 불가능한 손실 바구니로 노출을 집중시킨다"는 **실측으로 반증됐다.**

### 판정

- **H1은 기각이 아니다. 보류를 유지한다.** 생존편향은 실재하지만 작다
  (+14.74pp → +12.88pp). 문제는 부호가 아니라 **검정력**이다 — t가 3.71에서
  1.64로 떨어져 §3-2의 선택편향 보정(2,240 조합)을 통과하지 못한다.
- **전략은 −0.57%로 여전히 0 근처다.** 초판의 "−6%~−15%/건"은 철회한다.
  **폐쇄 후보 표시도 철회한다.**
- §7의 승격 gate는 그대로 유효하다. 다만 **"EXPIRED redeem 증거 적재"** 항목의
  성격이 바뀌었다 — 이제 필요한 것은 "전손인지 확인"이 아니라
  **표본을 늘려 t를 올리는 것**이다.
- §6의 리스크 상한(적용 확인 = `max_positions: 20`)과 청산 배리어 권고는 영향 없다.

### 부수 발견 — "EXPIRED n건 - 수동 redeem 필요" 경고는 오해를 부른다

auto-redeem이 켜져 있으면 이긴 포지션은 자동 상환된다. 그런데 봇은
**resolution 결과까지만 적재하고 redeemable 상태나 실제 redeem transaction을
수집하지 않는다**(L3 계약에 명시). 그래서 `EXPIRED`는 "해결됐고 우리가 상환을
기록하지 않은 것"이며, **이미 auto-redeem된 것과 진짜 미상환이 섞여 있다.**

실제로 승 7건 중 운영자가 UI에서 수동 상환한 것은 1건뿐이었다 — 나머지 6건은
auto-redeem이 처리한 것으로 보인다. 경고 문구를 `해결됨 — 상환 상태 미확인`으로
바꾸고, 진짜 backlog는 지갑 조회로 판정해야 한다.
...
> **2026-08-04 갱신**: §6-c에서 EXPIRED 해결 결과를 실측한 결과 H1의 우위는
> **+12.88pp(t=+1.64)** 로 축소되지만 유지된다. **기각이 아니라 검정력 부족**이므로
> 아래 설계는 유효하며, 승격 gate를 충족하면 개시한다.
