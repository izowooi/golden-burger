...
이 문서는 13개 `golden-*` 전략 회고의 공통 데이터 계약이다. 각 전략별 문서는
...
`golden-papaya`의 request envelope는 liquidity ≥ $1k이므로 중앙 archive가 저유동성
universe를 완전히 덮지 못한다. papaya counterfactual은 자체 archive(YES ≥ 0.80,
잔여시간 ≤168h, 최소 60일 보존)를 주 source로 사용하고, 중앙 archive는 교집합 대조용으로만
사용한다. papaya 자체 archive도 동일한 cursor-complete sweep/membership digest/catalog/event
coverage 계약을 지킨다.
