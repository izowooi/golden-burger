...
> 대상: Resolution Momentum 전략. 2026-07-22 현재 신규 진입은 비스포츠 `endDate` / 경기 전
> 스포츠 `gameStartTime` 기준 `0 < 잔여시간 <= 120h`다. 스포츠는 경기 시작 뒤에도 Gamma가
> `active=true`, `closed=false`, `enableOrderBook=true`, `acceptingOrders=true`로 보고하고
> `POLYBOT_ALLOW_IN_PLAY=true`이면 신규 진입할 수 있다. `POLYBOT_EXIT_HOURS=0`이므로 시간 청산은
> 비활성이다. 그 전 24~720h/12h 코호트는 대규모 손실 원인 분석을 위해 별도 historical
> cohort로 유지한다.
...
- 2026-07-22 계약 변경도 Git/config cohort로 분리한다: 0~120h 전환, 주문 직전 스포츠 상태
  재검증, 인플레이 진입 허용을 과거 24~720h/12h cohort와 합치지 않는다. 현재 스포츠 거래는
  `sports_phase_at_buy`의 `pregame`/`in_play`를 나눠 보고한다.
...
**논지**: "해결(resolution)이 가까워진 고확률 favorite은 1.0으로 수렴한다"에 베팅한다. Gamma API keyset cursor를 끝까지 순회해 주문 가능한 시장을 스캔하고, 확률 0.75~0.92 구간의 favorite 토큰을 매수한다. 2026-07-22 이후 진입 기준은 비스포츠 `endDate`까지 0~120h, 스포츠는 `gameStartTime`까지 0~120h 또는 주문 가능한 인플레이 상태다. 수익 원천은 "0.75~0.92 매수 → 1.0 방향 수렴"의 캡처(favorite-longshot bias + 시간가치 소멸)다. 한 번 거래하거나 `rapid_jump`로 영구 skip한 시장은 **같은 job DB 안에서** 재진입하지 않는다.

**진입 규칙** (`src/polybot/strategy/scanner.py::scan_buy_candidates` → `trader.py::execute_buy`):
1. Gamma의 `active=true`, `closed=false`, `enableOrderBook=true`, `acceptingOrders=true`를 모두 만족
2. 실효 유동성 >= `max(min_liquidity, buy_amount_usdc / max_order_liquidity_ratio)`
3. favorite 확률 p: `buy_threshold <= p <= sell_threshold` (양끝 포함). 기본 모드는 Yes/No 중 높은 쪽(→ **NO 토큰도 매수**), `--yes-only`면 index 0(Yes) 토큰만
4. 진입 시간: 비스포츠 `0 < endDate 잔여 h <= 120`; 스포츠는 경기 전 `0 < gameStartTime 잔여 h <= 120` 또는 인플레이
5. 매수 직전 CLOB midpoint 재검증: `> sell_threshold`면 "rapid_jump"로 **영구 skip**, `< buy_threshold`면 이번 사이클만 skip
6. 주문 직전 sports pregame/in-play 상태, open 원금/포지션/cycle burst, 주문/유동성 비율 재검증
...
**주문·증거 계약**: GTC의 `orderID`/`live`/`accepted`는 체결 증거가 아니다. submission,
order, exact trade association, partial fill을 execution ledger에 기록하고 매 cycle 대사한다.
대사 실패는 전체 전략의 주문 성공으로 추정하지 않으며 해당 token/side를 국소 격리한다. 증거
...
| 수명주기 모드 | `POLYBOT_LIFECYCLE_MODE` | active | `active`/`close_only`/`archive_only` |
...
- 자금 계좌는 golden-banana지만 strategy/run audit/DB 이름은 golden-cherry다.
- `POLYMARKET_PRIVATE_KEY`와 `POLYMARKET_FUNDER_ADDRESS`는 Jenkins Credentials Binding으로만
  주입한다. `POLYMARKET_SIGNATURE_TYPE`은 코드 기본 1이며 1/3만 허용한다. 기존에 주문이
  정상인 계좌는 임의로 변경하지 않는다.
- `uv run python main.py config`는 주문/API 호출 없이 resolved config를 검증·출력하는 선택적
  preflight다. `run`도 같은 검증을 반복하므로 필수는 아니지만 config 변경 직후에는 유지한다.
- `LOG_LEVEL`은 현재 Golden Cherry가 읽지 않는다. 기본 INFO이며 DEBUG는 `run --verbose`로 켠다.
- 현재 시간창은 **신규 BUY만 제한**한다. 기존 HOLDING을 창 밖이라는 이유로 매도하지 않으며,
  `POLYBOT_EXIT_HOURS=0`에서는 `time_exit`도 발생하지 않는다.
...
해석 가이드: 과거 12h 청산 cohort에서 `time_exit`이 어떤 역할을 했는지와, 현행
`POLYBOT_EXIT_HOURS=0` cohort에서 손익 분포가 어떻게 달라졌는지를 분리한다. 동시에
`trailing_stop`이 whipsaw로 손실을 양산하는지(실효 손절 -5% 문제), `stop_loss`가 거의
안 나오는지(트레일링에 선점) 확인한다.
...
### 3.4 잔여시간·스포츠 단계별 성과 — 과거/현재 진입 계약 교정 근거
...
```sql
-- 태그(카테고리)별 — 스포츠 카테고리 제외는 꺼져 있으므로 스포츠 성과를 분리 확인
...
| 창 (min~max) | 의미 |
|--------------|------|
| **0~120h** | **2026-07-22 이후 경기 전/비스포츠 현재값; 스포츠 gameStartTime / 비스포츠 endDate** |
...
인플레이는 시간창 스윕에 억지로 넣지 않는다. `POLYBOT_ALLOW_IN_PLAY`를 독립 노브로 두고
`sports_phase_at_buy='pregame'` 대 `in_play`의 confirmed-fill 성과를 비교한다. Gamma의
`gameStartTime`에는 경기 종료까지 남은 분이 없으므로 "후반 5분" 같은 반사실은 이 데이터만으로
재현할 수 없다.
...
- **인플레이 시계 부재**: `gameStartTime`은 kickoff 시각이지 경기 남은 시간 데이터가 아니다.
  `sports_phase_at_buy`로 경기 전/중만 나눌 수 있고 "종료 5분 전" 성과는 별도 live score feed 없이는 검증 불가.
- **endDate != 실제 해결 시각**: 조기 해결 시장은 time_exit 재생이 실제와 어긋난다. 스냅샷이 endDate보다 훨씬 먼저 끊긴 시장은 조기 해결로 간주하고 별도 표기.
- **Gamma liquidity != 실행 가능한 depth**: `liquidity`와 0.2% 비율은 1차 안전장치일 뿐
  실제 ask/bid depth와 slippage를 증명하지 않는다. $100을 넘는 scale-up 판단은 orderbook depth를 별도로 수집한 cohort만 사용한다.
...
- **과거 백로그 코호트**: 30일 진입창을 쓰던 과거 운영 첫 며칠에는 기존 후보를 일괄 매수하는 burst가 발생할 수 있었다. `buy_timestamp`와 config/Git hash로 "과거 초기 백로그"와 정상 신호를 분리하고, 현행 0~120h cohort에 합치지 않는다.
- **좀비로 인한 생존편향**: §3.5 — COMPLETED만 보면 패배가 과소 집계된다. 보정 P&L 없이는 어떤 교정 결론도 내리지 말 것.
- **스포츠 포함 + 단계 차이**: 현행 `excluded_categories: []`이므로 스포츠 시장이 포함된다. 다만
  스포츠는 gameStartTime 검증과 in-play 허용 계약을 거친다. 태그별 분해와 함께
  `sports_phase_at_buy`를 나눠 카테고리 효과와 경기 단계를 파라미터 효과로 오독하지 않는다.
...
| POLYBOT_ALLOW_IN_PLAY | true | | pregame/in_play confirmed-fill 분해 | |
| POLYBOT_MIN_LIQUIDITY | 50000 | | config cohort + liquidity_at_buy 분해 | |
| POLYBOT_MAX_ORDER_LIQUIDITY_RATIO | 0.002 | | orderbook depth/slippage coverage 있는 cohort만 | |
...
- 문서 생성: 2026-07-07. 마지막 계약 동기화: 2026-07-22 (branch main)
- 현재 계약 변경 계보: `34f61b1`(5일 창·불확실 주문 격리), `32c04b8`(주문 직전
  라이브 스포츠/노출 안전장치), `868314c`(주문 가능한 인플레이 진입 허용),
