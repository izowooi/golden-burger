...
전략: **Spread Harvest** — **수익원은 진입 신호가 아니라 실행 측면(maker/taker)이다.**
진입 신호(strict standard binary YES의 첫 관측 0.90 상향 교차, 0.90~0.94 매수)는
golden-queen에서 의도적으로 그대로 상속했다. 처치는 `execution_mode`
(passive/nearest/cross) 하나이며, 회고의 **1순위 집계 축**도 이것이다. 일반 시장은 종료까지 `(0h, 24h]`, 스포츠는 경기 전 또는 시작
후 360분 이내가 기본이다. 사전 해결 청산은 YES 0.98 목표와 0.85 절대 stop뿐이다.

## 0-A. quince 회고에서 반드시 먼저 볼 것

이 전략의 판정은 승률이 아니라 **같은 신호에 대한 순 실행 비용 차이**다.
...
1) quince 24h/12h DB를 각각 명시해 polybot-retro audit --strict를 실행한다.
...
export QUEEN_24_DB=/absolute/path/golden-quince/data/quince-live-24h/trades.db
export QUEEN_12_DB=/absolute/path/golden-quince/data/quince-live-12h/trades.db
export RETRO_OUTPUT="$HOME/polybot-retro/quince-$REVIEW_END"
```
...
```bash
uv run --project golden-quince python golden-quince/scripts/backtest.py \
  /absolute/path/quince-research.csv \
...
KEEP 조건에 더해 [Queen 증액 가이드](../../golden-quince/docs/SCALING_AND_TAIL_RISK.md)의
