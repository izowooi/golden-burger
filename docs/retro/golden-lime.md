...
**운영 env는 repo에 없다.** 실제 운영값은 Jenkins job 설정의 export 블록이 소스다 — 회고 시 §0 프롬프트에 (키 제외하고) 복사해 붙여넣을 것.
...
모든 봇이 같은 gamma sweep(가장 오래된 활성 시장 ~2100개)을 스냅샷하므로, **nectarine DB의 market_snapshots를 공용 아카이브**로 쓴다:
...
4. 체결 가정: 스냅샷 가격 = midpoint 체결 (실제 GTC limit 체결과 다름, §c 참조).
...
- **endDate 부재**: 아카이브에는 endDate가 없다. `entry_hours_min`(해결까지 >= 24h) 게이트는 "t 이후 24h 이상 스냅샷이 존속했는가"로 근사하거나 gamma API로 endDate를 보충한다. time_exit 재현도 동일 근사.
...
- **체결 가정**: 실봇은 GTC limit at midpoint + 접수=체결 가정(STRATEGY.md §8)이라 기록 P&L 자체가 과대평가일 수 있고, 반사실도 같은 가정을 쓴다. 절대값보다 격자 간 상대 비교로 해석하라.
...
- **코호트 분리**: (1) 운영 초기 백로그 코호트 — 거래량 게이트는 백필 불가라 실축적 스냅샷이 쌓인 뒤에야 진입이 시작되고, 초기 vol_mult는 얕은 윈도우 평균 기반이라 왜곡 가능. 운영 첫 24~48h 거래는 분리 집계. (2) `mode` 컬럼으로 sim/live 분리 — 절대 합산하지 않는다. (3) Jenkins env 변경이 있었다면 변경 시점 전후를 분리 (export 블록 이력 확인).
...
**라운드 절차**: 제안값 반영은 **Jenkins env만 바꾸면 된다** (코드 수정 불필요, env > yaml > 기본값). 제안값으로 **2차 테스트 4주 → 재회고 → 3차 교정**. cherry처럼 기본/변형 병행 운용도 가능: 현행 유지 job + 제안값 job(`--job lime-a1` 등, job별 DB 자동 분리)을 동시에 돌려 직접 A/B 한다. 변형 프리셋(민감 A-1 / 보수 A-2 / 빠른 회전 A-3)은 STRATEGY.md §7 참조.
