...
- 위 `REVIEW_START`는 진입 허용 계약의 시작이고 실제 수집 시작은 배포 시각이다. v1의
  기존 first run 범위를 v2에 재사용하지 않는다. v2 `preregistration_sha256`을 가진 각 job의
...
3. source `live/ended/elapsed/period`가 kickoff부터 종료까지 이어지는지 점검한다. source minute
   및 wall-clock age 상한은 없어야 하며 예정 kickoff로 시계를 보간하지 않는다.
...
8. Silver의 `execution_capacity_json`이 모든 저장 snapshot에 있고 5/10/25/50/100/250/500
...
- 우선순위 `target → entry−0.15 stop`, 그 외에는 검증된 resolution까지 유지하는 경로를
  재생한다. source minute 기반 강제 청산이 제출되지 않았는지 확인한다.
