# 2026-09-26 이전 루트 AGENTS.md 보존본

이 파일은 GPT-6 전환 전의 상세 인덱스를 보존한다. 전략명·금액·Jenkins 배치·수집기 상태는 이후 바뀌었을 수 있으므로 **현재 운영의 권위가 아니다**. 현재 값은 해당 프로젝트 AGENTS.md와 verified DB resolved config, Jenkins effective config에서 확인한다.
