...
| **day 1** | 각 팔의 **첫 CONFIRMED 체결**에서 §6의 kill-check 3개 확인 | 즉시 중단·수정·재시작 (표본 리셋) |
...
### day-1 kill-check — 첫 CONFIRMED 체결에서 반드시 확인

이 셋 중 하나라도 실패하면 **30일을 돌려도 아무것도 측정되지 않는다.** 30일 뒤에
발견할 일이 아니다.
