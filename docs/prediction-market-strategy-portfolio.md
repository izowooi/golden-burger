...
기존 3개 + 신규 10개 = 총 13개 전략의 전체 지도. 각 전략의 상세 근거와 규칙은 각 폴더의 `STRATEGY.md`, 기존 봇 분석은 각 폴더의 `STRATEGY_ANALYSIS.md`, 사람이 읽기 좋은 HTML 버전은 `docs/strategy-pages/`, 회고 절차는 `docs/ab-retro-playbook.md` 참조.
...
| golden-papaya | Final Five | 95% first observed crossing 뒤 해결 수렴 가설 | strict binary YES 편승 | 관측된 0.95 교차, 2–72h | 신규 (falsification cohort) |

포트폴리오 관점의 커버리지:

- **시간축**: 마감 임박(cherry/date/papaya) ↔ 마감 먼 구간(elderberry/grape) ↔ 시간 무관(honeydew)
...
표준 이진 YES가 해결 2–72시간 전에 archive cadence에서 **first observed upward crossing**으로
0.95를 돌파하면 0.95–0.97에서만
진입하고, 사전 해결 익절·time exit 없이 resolution/redeem까지 보유한다. 수렴 실패 신호는
절대 YES 0.90 stop 하나뿐이다. $1k 유동성/$0 volume 기본값은 저유동성 영역까지 가설을
기각하기 위한 넓은 관측 envelope이며 안전 보장이 아니다. fresh best ask가 0.97을 넘으면
진입하지 않고, stop은 fresh best bid와 confirmed fill로 실행 가능성을 평가한다. 중앙 archive가
이 저유동성 universe를 덮지 못하므로 YES≥0.80·잔여≤168h 자체 archive를 60일 보존한다.
...
| 해결된 시장이 영원히 HOLDING | 일반 신규 전략은 endDate+24h 경과 시 EXPIRED 처리 + 수동 redeem 경고. papaya는 예외로 time exit/수익성 EXPIRED 처리를 하지 않고 resolution·redeem evidence를 분리 기록 |
...
py-clob-client-v2, 1실행=1사이클. 기존 신규 전략은 GTC midpoint 흐름을 유지하지만 papaya는
작은 edge와 낮은 유동성을 숨기지 않기 위해 주문 직전 fresh best ask 상한을 진입 게이트로
사용하고, 진입·성과 확정은 별도의 confirmed fill evidence로만 수행한다.
...
2차 3종은 월 2개 페이스로 병행 투입 가능: **mango**는 근거가 가장 강해 M1~M2에 date와 병행 A/B를 권장, **orange**는 fig/lime과 같은 계열이므로 M2~M3에, **nectarine**은 백테스트 복제 검증 목적으로 아무 때나 소액 병행. **papaya**는 기존 slot/DB를 재사용하지 않는 별도 simulation → $5 live falsification cohort로 마지막에 추가하고, 4주와 terminal event 유효 n 30 이전에는 증액하지 않는다.

### A/B 판단 기준 (제안)

- 각 전략 최소 **4주 + 30건 이상** 거래 후 판단. 단, papaya는 단순 trade 수가 아니라 **terminal event-cluster 유효 n 30**을 요구한다.
...
- GTC 주문의 `accepted`/order ID는 fill이 아니다. 실제 성과는 confirmed fill evidence로만 확정하며, papaya의 stop SELL은 접수 뒤 `PENDING_SELL`을 유지하고 exact BUY/SELL fill 대사 전에는 완료로 간주하지 않는다.
