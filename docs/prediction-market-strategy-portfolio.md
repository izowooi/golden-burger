...
exit은 없다. 첫 달은 $5, 24h 대 12h 한 축만 별도 계정·job·DB에서 사전 등록 A/B한다.
...
2차 3종은 월 2개 페이스로 병행 투입 가능: **mango**는 근거가 가장 강해 M1~M2에 date와 병행 A/B를 권장, **orange**는 fig/lime과 같은 계열이므로 M2~M3에, **nectarine**은 백테스트 복제 검증 목적으로 아무 때나 소액 병행. **papaya**는 기존 slot/DB를 재사용하지 않는 별도 simulation → $5 live falsification cohort로 마지막에 추가한다. **queen**도 별도 계정·job·DB에서 24h/12h만 바꾼 $5 A/B로 시작한다. 둘 다 4주와 terminal event 유효 n 30 이전에는 증액하지 않는다.
