...
def _validate_config(trading: TradingConfig, api: ApiConfig) -> None:
    ...
    numeric = {
        ...
        "strategy.exclude_recent_hours": strategy.exclude_recent_hours,
        "strategy.hold_hours": strategy.hold_hours,
        "strategy.prob_min": strategy.prob_min,
        "strategy.prob_max": strategy.prob_max,
    ...
    if strategy.lookback_days <= 0 or strategy.hold_hours <= 0:
        raise ValueError("strategy lookback_days and hold_hours must be > 0")
    if not 0 < strategy.exclude_recent_hours < strategy.lookback_days * 24:
        raise ValueError("strategy.exclude_recent_hours must be > 0 and shorter than lookback_days")
    if not 0 < strategy.prob_min < strategy.prob_max < 1:
        raise ValueError("strategy probability bounds must satisfy 0 < prob_min < prob_max < 1")
