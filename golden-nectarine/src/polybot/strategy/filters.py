...
# 주의: "game", "match", "win the" 등 광범위한 키워드로 과차단 문제가 있어
# excluded_categories 기본값은 빈 배열(필터 비활성)이다.
SPORTS_KEYWORDS = [
    # Leagues
...
def passes_volume_filter(market: Dict, min_volume_24h: float) -> bool:
    ...
    Args:
        market: Market dictionary (gamma의 volume24hr 필드 사용)
...
def get_yes_price(market: Dict) -> float | None:
    """마켓의 YES 가격 조회 (outcomePrices[0]).

    스냅샷 히스토리는 항상 YES 가격 기준으로 저장한다.

    Args:
        market: Market dictionary with outcomePrices

    Returns:
        YES 가격 (0.0-1.0) 또는 파싱 불가 시 None
    """
