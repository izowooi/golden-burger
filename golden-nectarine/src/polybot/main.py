...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Polymarket Automated Trading Bot (Golden Nectarine - Bottom Fisher)",
...
Examples:
  ...
  polybot config
    ...
    if args.command == "run":
        ...
        print(f"Min 24h Volume: ${config.trading.min_volume_24h:,.0f} (0 = 비활성)")
        print(f"Max Positions: {config.trading.max_positions if config.trading.max_positions > 0 else 'Unlimited'}")
        print(f"Take Profit (안전판): {config.trading.take_profit_percent:+.0%} (0.99 캡)")
        print(f"Stop Loss (안전판): {config.trading.stop_loss_percent:+.0%}")
        print(f"Reentry Cooldown: {config.trading.reentry_cooldown_hours}h")
        print(f"History Backfill: {config.trading.history_backfill} (20일 룩백의 생명선)")
        print()
        print("=== Bottom Fisher Strategy ===")
        print(f"Lookback: {s.lookback_days}일 (최근 {s.exclude_recent_hours}h 제외)")
        print(f"Entry Band: [{s.prob_min:.0%}, {s.prob_max:.0%}] (YES 매수 고정)")
        print(f"Calendar Exit: 보유 {s.hold_hours}h (주 청산 경로)")
        print()
        print("=== Time-based ===")
        print(f"Entry Hours Min: {config.trading.time_based.entry_hours_min}h (30일+ 장기 시장만)")
