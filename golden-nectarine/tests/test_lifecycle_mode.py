...
def _build_bot(monkeypatch, tmp_path, lifecycle_mode: str, holdings):
    scanner = MagicMock()
    scanner.fetch_markets.return_value = [{"condition_id": "market-1"}]
...
def test_close_only_checks_exits_but_never_scans_or_buys(monkeypatch, tmp_path):
    ...
    repo.save_cycle_stats.assert_not_called()
...
def test_active_keeps_existing_entry_path(monkeypatch, tmp_path):
    ...
    candidate = {"condition_id": "market-1", "question": "test"}
    scanner.scan_buy_candidates.side_effect = None
    scanner.scan_buy_candidates.return_value = [candidate]
    repo.get_position_count.return_value = 0
    repo.is_reentry_blocked.return_value = (False, "")
    trader.buying_disabled = False
    ...
    repo.save_cycle_stats.assert_called_once()
    session.close.assert_called_once()
