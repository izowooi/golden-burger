...
from polybot.config import ApricotEntryConfig, TradingConfig
...
def test_entry_relative_stop_uses_frozen_ten_point_reversal() -> None:
    ...
    # The frozen Apricot stop is ten percentage points below confirmed entry.
    trade = SimpleNamespace(
        id=9,
        condition_id="condition-1",
        event_id="event-1",
...
def test_gamma_void_resolution_uses_exact_selected_token_half_payout() -> None:
    repo, clob = _Repo(), _Clob()
    gamma = SimpleNamespace(
        get_market_by_condition_id=lambda _condition: {
            "conditionId": "condition-1",
    ...
    assert update["resolution_status"] == "gamma_closed_void_0_5"
    assert update["settlement_pnl_assumption"] == pytest.approx(
