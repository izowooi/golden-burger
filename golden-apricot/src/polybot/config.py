"""Resolved configuration for the Golden Apricot kickoff-leader experiment."""
...
TICK50_START_UTC = "2026-09-10T13:00:00Z"
TICK50_ENTRY_END_UTC = "2026-09-24T13:00:00Z"
...
FROZEN_JOB_TAKE_PROFIT = {
    ...
    "apricot-live-fruit-sixbook-net5-sl12-75m-v2": 0.05,
    "apricot-live-eco-mlb-7pp-20sl-1m-v1": 0.07,
    "apricot-live-fruit-mlb-10pp-20sl-1m-v1": 0.10,
    "apricot-shadow-1m-v1": 0.05,
    "apricot-shadow-mlb-1m-v2": 0.05,
    "apricot-shadow-nba-1m-v2": 0.05,
    "apricot-shadow-nfl-1m-v2": 0.05,
...
FROZEN_JOB_SPORT_FAMILY = {
    ...
    "apricot-live-fruit-sixbook-net5-sl12-75m-v2": "soccer",
    "apricot-live-eco-mlb-7pp-20sl-1m-v1": "mlb",
    "apricot-live-fruit-mlb-10pp-20sl-1m-v1": "mlb",
    "apricot-shadow-1m-v1": "soccer",
    "apricot-shadow-mlb-1m-v2": "mlb",
    "apricot-shadow-nba-1m-v2": "nba",
    "apricot-shadow-nfl-1m-v2": "nfl",
    "apricot-shadow-nhl-1m-v2": "nhl",
}
FROZEN_JOB_PROFILE_KEY = {
    job: (
        "mlb_live"
        if job in {
            "apricot-live-eco-mlb-7pp-20sl-1m-v1",
            "apricot-live-fruit-mlb-10pp-20sl-1m-v1",
        }
        else family
    )
    for job, family in FROZEN_JOB_SPORT_FAMILY.items()
}
FROZEN_JOB_PROFILE_KEY.update({job: "mlb_live" for job in TICK50_JOBS})
...
FROZEN_JOB_STOP_LOSS.update({job: 0.99 for job in TICK50_JOBS})
FROZEN_JOB_STOP_LOSS.update({
    "apricot-live-eco-sixbook-net5-sl15-75m-v2": 0.15,
    "apricot-live-fruit-sixbook-net5-sl12-75m-v2": 0.12,
})
...
FROZEN_JOB_EXPERIMENT_DATES = {
    job: (
        (TICK50_START_UTC, TICK50_ENTRY_END_UTC, TICK50_FOLLOWUP_END_UTC)
        if job in TICK50_JOBS
        else (SIX_BOOK_NET_START_UTC, SIX_BOOK_NET_ENTRY_END_UTC, SIX_BOOK_NET_FOLLOWUP_END_UTC)
...
FROZEN_SIMULATION_JOBS = frozenset()
FROZEN_JOB_EXPERIMENT_DATES = {
    job: (TICK50_START_UTC, TICK50_ENTRY_END_UTC, TICK50_FOLLOWUP_END_UTC)
    for job in TICK50_JOBS
}
...
CLASSIFIER_VERSION = "apricot-major-sports-family-contract-v2"
...
SPORT_PARAMETER_PROFILES = {
    "soccer": SportParameterProfile(
        code="soccer",
        profile_version="apricot-soccer-kickoff-v3",
    ...
    **{
        family: SportParameterProfile(
            code=family,
            profile_version=f"apricot-{family}-shadow-ready-v1",
...
EntryConfig = ApricotEntryConfig
...
@dataclass
class TradingConfig:
    """Golden Apricot live/shadow trading and evidence configuration."""
    ...
    entry: ApricotEntryConfig = field(default_factory=ApricotEntryConfig)
    archive: ArchiveConfig = field(default_factory=ArchiveConfig)
    excluded_categories: List[str] = field(default_factory=list)

    @property
    def strategy(self) -> ApricotEntryConfig:
        return self.entry
...
def _validate_config(
    ...
    *,
    ...
    if profile is None:
        raise ValueError(f"unsupported Golden Apricot runtime job: {job_name}")
    numeric = {
    ...
    if (
        ...
        raise ValueError(
            "Golden Apricot liquidity gate is frozen at $5k cumulative "
    ...
    if (
        ...
        or entry.exit_basis != TICK50_JOBS.get(
            job_name, "net_return" if job_name in SIX_BOOK_NET_JOBS else "absolute_delta"
        )
    ...
    if job_name in TICK50_JOBS and (
        trading.book_shape != "direct-two-team-moneyline"
...
def load_config(
    ...
    if profile.code != resolved_sport_family:
        raise ValueError("Golden Apricot runtime sport/profile identity mismatch")
