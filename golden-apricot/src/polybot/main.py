"""CLI entry point for Golden Apricot."""
...
def _record_simulation_failure(config, error: BaseException) -> None:
    ...
    if not config.simulation_mode:
        return
    try:
        audit = RunAudit.start(config, strategy_name="golden-apricot")
        audit.fail(error)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Golden Apricot - in-play direct-book leader A/B"
    )
...
def main() -> None:
    ...
    print("=== Golden Apricot / In-Play Direct-Book Leader ===")
