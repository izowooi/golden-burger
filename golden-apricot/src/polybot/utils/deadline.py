"""Mode-aware cooperative budget for one Golden Apricot cycle.

Simulation collectors stop starting Gamma/CLOB work eight seconds before the
50-second hard boundary. Live Eco/Fruit cycles retain warning-only behavior:
elapsed time never suppresses reconciliation, exits, or a POST boundary.
"""
...
class CycleBudget:
    """One monotonic runtime budget shared by the complete cycle."""
    ...
    def ensure_can_start_request(self, context: str) -> None:
        """Reject new simulation HTTP work after the 42-second boundary."""
