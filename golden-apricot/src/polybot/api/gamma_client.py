...
class GammaClient:
    ...
    def __init__(
        ...
        *,
        ...
        if normalized_family not in SPORT_FAMILY_TAG_IDS:
            raise ValueError(
                f"unsupported Golden Apricot sport family: "
        ...
        if sport_profile_version is None:
            ...
            if len(matching_profiles) != 1:
                raise ValueError(
                    "Golden Apricot sport profile version does not uniquely match "
        ...
        self.session.headers.update(
            {
                ...
                "User-Agent": "GoldenApricot-PolyBot/1.0",
            }
        )
    ...
    def get_all_tradable_markets(
        ...
        logger.info(
            "Golden Apricot %s sweep - events=%s markets=%s "
