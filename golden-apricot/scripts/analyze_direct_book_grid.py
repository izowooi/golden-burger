#!/usr/bin/env python3
"""Replay one explicit Golden Apricot cohort's direct CLOB books, read-only.
...
def _load_contract(connection: sqlite3.Connection, cohort: Cohort) -> dict:
    ...
    if (
        row["strategy_name"] != "golden-apricot"
        or config.get("strategy_name") != "golden-apricot"
    ...
    if not connection.execute(
        "SELECT 1 FROM run_audits WHERE config_hash=? AND job_name=? AND mode=? AND strategy_name='golden-apricot' LIMIT 1",
...
def _same_run(row: sqlite3.Row, cohort: Cohort) -> bool:
    return (
        ...
        and row["run_strategy"] == "golden-apricot"
    )
...
def _read_paths(
    ...
    *,
    ...
    boundaries = connection.execute(
        ...
        "WHERE job_name=? AND mode=? AND strategy_name='golden-apricot' "
...
def list_cohorts(path: Path) -> list[dict]:
    with sqlite3.connect(f"file:{path.resolve()}?mode=ro", uri=True) as connection:
        ...
        return [
            ...
            WHERE r.strategy_name='golden-apricot'
...
def analyze(
    ...
    *,
    ...
    return {
        "schema": "golden-apricot-direct-book-grid-v2",
