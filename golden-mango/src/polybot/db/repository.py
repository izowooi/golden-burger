...
class TradeRepository:
    ...
    def cleanup_old_snapshots(self, days: int = 7) -> int:
        ...
        Returns:
            삭제된 스냅샷 수
        """
        if compact_maintenance_active(self.session, "golden-mango"):
            return 0
    ...
    def append_trade_to_csv(self, trade: Trade, db_dir) -> None:
        """완료된 거래를 월별 CSV 파일에 추가 (시그널 컬럼 포함, 부록 §A).
        ...
        headers = [
            ...
            "volume_24h_at_buy",
            "carry_yield_at_buy", "momentum_6h_at_buy", "carry_yield_at_exit",
            "market_tags",
        ]
        ...
        row = {
            ...
            "carry_yield_at_buy": trade.carry_yield_at_buy if trade.carry_yield_at_buy is not None else "",
            "momentum_6h_at_buy": trade.momentum_6h_at_buy if trade.momentum_6h_at_buy is not None else "",
            "carry_yield_at_exit": trade.carry_yield_at_exit if trade.carry_yield_at_exit is not None else "",
