"""Main bot orchestrator with Patience Premium strategy."""
import logging
...
class PolymarketBot:
    """Polymarket automated trading bot with Patience Premium strategy.

    Orchestrates the trading cycle:
    0. 스캔 대상 시장 스냅샷 저장 (모멘텀 가드 데이터)
        ...
        self.Session = init_database(
            ...
            SQLiteMaintenanceRequirements(
                full_cadence_hours=float(
                    config.trading.momentum_gate.lookback_hours
                )
            ),
        )
    ...
    def _log_strategy_config(self):
        ...
        logger.info(
            f"Patience Premium 전략 - "
            f"진입: {carry.entry_hours_min}h < 해결시간 <= {carry.entry_hours_max}h, "
            f"확률 [{carry.prob_min:.3f}, {carry.prob_max:.3f}], "
            f"연환산 캐리 y = ((1-p)/p)*(8760/h) >= {carry.yield_min:.2f}, "
            f"모멘텀 가드: 최근 {gate.lookback_hours}h favorite 변화 >= {gate.min_change:+.3f}"
        )
        logger.info(
            f"손익 설정 - 손절: {trading.stop_loss_percent:.0%}, "
            f"익절: 목표가 0.99 도달 (수렴 보유, trailing 없음), "
    ...
    def run_cycle(self) -> dict:
        ...
        try:
            ...
            # Phase 0: Save market snapshots (모멘텀 가드 데이터 축적)
    ...
    def run(self):
        ...
        audit = RunAudit.start(self.config, strategy_name="golden-mango")
        ...
        try:
            ...
            return {
                ...
                "holdings": [
                    {
                        ...
                        "carry_yield_at_buy": t.carry_yield_at_buy,
                ...
                "config": {
                    ...
                    "yield_min": trading.carry.yield_min,
                    "prob_min": trading.carry.prob_min,
                    "prob_max": trading.carry.prob_max,
                    "entry_hours_min": trading.carry.entry_hours_min,
                    "entry_hours_max": trading.carry.entry_hours_max,
