"""Market scanner for the Patience Premium strategy."""
import logging
import re
...
def _reason_key(reason: str) -> str:
    ...
    예: yield_below_min_0.18 → yield_below_min, too_early_350.3h → too_early
    """
...
class MarketScanner:
    """Scans markets for buy candidates (Patience Premium).
    ...
    def save_market_snapshots(self, markets: List[Dict]) -> int:
        """스캔 대상 시장의 스냅샷 저장 (모멘텀 가드 데이터 축적).
    ...
    def _log_scan_summary(self, analysis: List[Dict]):
        ...
        logger.info("=" * 70)
        logger.info("스캔 요약 (Patience Premium Strategy)")
        logger.info("=" * 70)

        carry = self.config.carry
        gate = self.config.momentum_gate
        logger.info(
            f"설정: {carry.entry_hours_min}h < 해결시간 <= {carry.entry_hours_max}h, "
            f"확률 [{carry.prob_min:.3f}, {carry.prob_max:.3f}], "
            f"연환산 캐리 y >= {carry.yield_min:.2f}, "
        ...
        for item in analysis:
            ...
            yield_str = f"y={item['carry_yield']:.2f}" if item.get("carry_yield") is not None else "y=N/A"

            logger.info(
                f"{status} | {item['outcome']} @ {item['probability']:.1%} | "
                f"{yield_str} | 해결까지: {hours_str} | 사유: {item['reason']}"
            )
    ...
    def scan_buy_candidates(self, markets: List[Dict]) -> List[Dict]:
        ...
        Criteria (Patience Premium Strategy):
        1. Not in excluded categories
        2. Liquidity >= min_liquidity (+ 옵션 volume24hr >= min_volume_24h)
        3. Favorite side 선택 (YES/NO 중 높은 쪽, --yes-only 지원)
        4. 캐리 진입: 6 < h <= 336, p ∈ [0.85, 0.985],
           y = ((1-p)/p)*(8760/h) >= y_min
        5. 모멘텀 가드: 최근 lookback 윈도우 favorite 변화 >= min_change
        ...
        for market in markets:
            ...
            # Filter: Liquidity (double check) + 옵션 24h volume
            if not passes_liquidity_filter(market, self.config.min_liquidity):
                rejected["low_liquidity"] = rejected.get("low_liquidity", 0) + 1
                continue
            if self.config.min_volume_24h > 0 and not passes_volume_filter(
                market, self.config.min_volume_24h
            ):
            ...
            # 캐리 조건 선검사 (실패 시 스냅샷/백필 조회를 생략해 API 낭비 방지)
            ...
            scan_analysis.append({
                ...
                "probability": probability,
                "hours_left": hours_left,
                "carry_yield": decision.carry_yield,
            ...
            logger.debug(
                ...
                f"({candidate['outcome']} @ {probability:.1%}, y={decision.carry_yield:.2f}, "
