...
- **운영 결과: 14일간 -$10.84** ($300.11 → $289.27). 그 손실의 **거의 전부가 `realized_pnl=NULL`인 EXPIRED 6건**이라 봇의 자체 손익 집계에는 보이지 않았다.

- **허들이 시간가치가 아니라 손실확률을 탐지한다.** `y >= 2.0`을 만족하려면 자본 기회비용(연 20% 가정)으로 정당화되는 할인의 **정확히 10배**가 필요하다 — 잔여시간과 무관하게 10배다. 순수한 lock-up 할인 시장은 y ≈ 0.2라 전부 탈락하고, 통과한 것은 **우리가 모르는 위험의 프리미엄**이다. `STRATEGY.md §5.3`이 이미 이 가능성을 자인했고, 데이터가 그쪽을 지지한다.

- **허들은 병목이 아니었다.** 로그의 제외 사유는 `too_early: 2103` vs `yield_below_min: 28`. 실제 병목은 `max_positions=10`이고, `최대 포지션 수 도달` 로그가 **749,321회** 찍혔다.

- **후보 정렬이 없다.** 매 사이클 127~213개 후보를 Gamma 스윕 순서대로 선착순 배정한다. 보유 10개는 **임의의 표본**이지 수익률 상위 10개가 아니다. "시장이 스스로 진입 frontier를 형성한다"는 전제가 코드에 구현된 적이 없다.

- **자체 기준 전부 미달**: 승률 54.5%(기준 85%), stop_loss 비중 42%(기준 15% 이하), resolved_unredeemed 6건(기준 0건).
