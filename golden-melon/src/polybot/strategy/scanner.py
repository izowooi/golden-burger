"""Gamma universe archive and threshold-crossing scanner for Resolution Sprint."""
...
class MarketScanner:
    ...
    def __init__(
        ...
        self.config = config
        self.repo = repo
        # Kept in the signature for shared construction code; Resolution Sprint relies
    ...
    def save_market_snapshots(
        ...
        if self.repo is None:
            raise RuntimeError("repository is required for Resolution Sprint archive evidence")
        ...
        try:
            ...
            raise
        logger.info(
            "Resolution Sprint research snapshot %s개 저장 (strict binary, YES>=%.2f, <=%.0fh)",
            saved,
    ...
    def _entry_snapshot_lineage(
        ...
        # Sixty-day retention is much wider than Melon's three-day archive.
    ...
    def scan_buy_candidates(
        ...
        """Return markets that have just crossed into the Resolution Sprint band."""
        reference = now or datetime.now(timezone.utc)
        if reference.tzinfo is None:
            reference = reference.replace(tzinfo=timezone.utc)
        ...
        logger.info("Resolution Sprint 매수 후보 %s개 발견", len(candidates))
        return candidates
