"""Order execution and evidence-safe settlement handling for Resolution Sprint."""
...
class Trader:
    ...
    def execute_buy(self, candidate: dict) -> Optional[int]:
        ...
        if candidate.get("outcome") != "Yes":
            logger.error("Resolution Sprint가 YES 이외 후보를 거부했습니다: %s", condition_id)
            return None
        ...
        if candidate.get("is_sports"):
            ...
            # Melon is intentionally falling back to endDate.
        ...
        logger.info(
            "Resolution Sprint 매수: '%s' signal=%.2f%% ask=%.2f%% "
    ...
    def _place_sell_with_balance_retry(
        ...
        *,
        ...
        Melon deliberately does not retry a smaller SELL after a balance
