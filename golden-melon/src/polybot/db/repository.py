"""Repository operations for Resolution Sprint trades and research evidence."""
...
class TradeRepository:
    ...
    def cleanup_old_snapshots(self, days: int = 60) -> int:
        if compact_maintenance_active(self.session, "golden-melon"):
            return 0
        ...
        self.session.execute(
            text(
                ...
                "_polybot_melon_protected_snapshots "
                "(id INTEGER PRIMARY KEY) WITHOUT ROWID"
            )
        )
        self.session.execute(text("DELETE FROM _polybot_melon_protected_snapshots"))
        self.session.execute(
            text(
        ...
        deleted = self.session.execute(
            text(
                ...
                "AND id NOT IN ("
                "SELECT id FROM _polybot_melon_protected_snapshots)"
            ),
            {"cutoff": cutoff},
        ).rowcount
        self.session.execute(text("DROP TABLE _polybot_melon_protected_snapshots"))
        expired_sweeps = [
            row[0]
