"""Resolved configuration for Golden Melon's Resolution Sprint strategy."""
...
def _get_execution_mode(yaml_value) -> str:
    ...
    **melon의 처치축이 아니다.** 실행 측면은 golden-quince가 A/B/C로 별도 검정
    중이므로, melon은 기존 봇과 같은 중립값 `nearest`로 고정해 거래량 gate 하나만
    처치로 남긴다. 두 실험이 같은 축을 동시에 흔들지 않게 하기 위한 것이다.
    """
...
class CrownEntryConfig:
    """Pure Resolution Sprint entry/exit thresholds.
...
@dataclass
class TradingConfig:
    """Resolution Sprint trading and evidence-capture configuration."""
...
def _validate_config(trading: TradingConfig, api: ApiConfig) -> None:
    ...
    if not trading.yes_only_mode:
        raise ValueError("Resolution Sprint inherently requires yes_only_mode=true")
...
def load_config(
    ...
    strategies.  Passing ``False`` is rejected; Melon never permits NO-side
    trading.
    """
    ...
    trading = TradingConfig(
        ...
        min_volume_24h=_get_config_value(
            "POLYBOT_MIN_VOLUME_24H", trading_cfg.get("min_volume_24h"), 50_000.0
        ),
