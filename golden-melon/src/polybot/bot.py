"""Resolution Sprint trading-cycle orchestration."""
...
class PolymarketBot:
    ...
    def _log_strategy_config(self) -> None:
        ...
        logger.info(
            "Resolution Sprint 전략 - prior YES < %.2f -> current YES [%.2f, %.2f], "
            "scheduled hours (%.1f, %.1f], absolute target >= %.2f / stop <= %.2f",
            entry.prob_min,
            entry.prob_min,
            entry.prob_max,
            entry.hours_min,
        ...
        logger.info(
            "리스크 - open notional <= $%.2f, positions=%s, event=%s, "
            "new/cycle=%s; exact excluded tags=%s; sports in-play=%s max=%.0fmin",
            trading.max_open_notional_usdc,
            trading.max_positions,
            trading.max_event_positions,
            trading.max_new_positions_per_cycle,
            trading.excluded_categories or "(none; sports included)",
            trading.sports.allow_in_play,
    ...
    def run_cycle(self) -> dict:
        ...
        try:
            ...
            logger.info("=== Phase 0: Resolution Sprint research archive ===")
    ...
    def run(self) -> None:
        ...
        audit = RunAudit.start(self.config, strategy_name="golden-melon")
        try:
            ...
            return {
                ...
                "config": {
                    ...
                    "entry": {
                        ...
                        "take_profit_price": (
                            trading.entry.take_profit_price
                        ),
                    ...
                    "sports": {
                        "included_when_no_exclusion_env": True,
                        "excluded_categories_exact": list(
                            trading.excluded_categories
                        ),
                        "use_game_start_time": (
                            trading.sports.use_game_start_time
                        ),
                        "allow_in_play": trading.sports.allow_in_play,
                        "reject_without_game_start": (
                            trading.sports.reject_without_game_start
                        ),
                        "max_in_play_minutes": (
                            trading.sports.max_in_play_minutes
                        ),
