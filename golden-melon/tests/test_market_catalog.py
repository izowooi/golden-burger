"""Resolution Sprint replay catalog, broad archive, and atomic sweep evidence."""
...
def activate_compact_maintenance(session, *, active=1):
    ...
    report = json.dumps(
        {
            "policy": asdict(policy_for("golden-melon", requirements)),
...
def test_window_rejected_first_crossing_is_durable_after_entry_window_opens(
    ...
    # Keep one fixed resolution time. The first crossing is outside Melon's
