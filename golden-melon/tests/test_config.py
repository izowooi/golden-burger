"""Golden Melon config precedence, defaults, and DB isolation."""
...
def test_crown_cross_defaults_are_fail_closed():
    config = load_config("missing.yaml", job_name="quince")
    trading = config.trading

    assert trading.lifecycle_mode == "active"
    assert trading.buy_amount_usdc == 5.0
    assert trading.min_liquidity == 20_000.0
    assert trading.min_volume_24h == 50_000.0
    ...
    # Resolution Sprint 고유 계약: 실행 모드는 처치축이 아니라 기존 봇과 같은
    ...
    assert trading.experiment_capital_usdc == 100.0
    assert trading.effective_min_liquidity == 20_000.0
    assert trading.effective_min_volume_24h == 50_000.0
...
@pytest.mark.parametrize(
    ("amount", "liquidity", "volume", "open_notional"),
    [
        ("5", 20_000.0, 50_000.0, 50.0),
        ("100", 100_000.0, 50_000.0, 1_000.0),
        ("200", 200_000.0, 50_000.0, 2_000.0),
        ("400", 400_000.0, 50_000.0, 4_000.0),
        ("1000", 1_000_000.0, 50_000.0, 10_000.0),
    ],
)
def test_single_size_control_scales_risk_gates(
    monkeypatch,
    amount,
    liquidity,
    volume,
    ...
    assert trading.effective_min_liquidity == liquidity
    assert trading.effective_min_volume_24h == volume
    assert trading.max_open_notional_usdc == open_notional
