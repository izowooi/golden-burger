"""Resolution Sprint order-book execution and settlement-evidence contracts."""
...
def make_trade(**overrides):
    trade = SimpleNamespace(
        ...
        stop_price_at_entry=0.78,
        take_profit_price_at_entry=0.97,
...
class TestDrawdownKillSwitchBlocksEntry:
    """사전 등록 낙폭 한도가 실제로 진입을 막는지 — 예측이 아니라 실행 경로 검증."""

    def test_entry_blocked_once_realized_loss_hits_limit(self):
        clob = FakeClob(midpoint=0.911, best_bid=0.90, best_ask=0.91)
        repo = FakeRepo()
        # 기본 실험자금 $200 x 20% = $40 한도
        repo.realized_pnl_total = -40.0
        trader, _, _ = make_trader(clob=clob, repo=repo, simulation=False)

        assert trader.execute_buy(make_candidate()) is None
        assert clob.orders == []

    def test_entry_proceeds_just_under_limit(self):
        clob = FakeClob(midpoint=0.911, best_bid=0.90, best_ask=0.91)
        repo = FakeRepo()
        repo.realized_pnl_total = -19.99
        trader, _, _ = make_trader(clob=clob, repo=repo, simulation=False)
...
class TestEntryExecution:
    def test_buy_revalidates_crossing_and_executes_with_depth_capped_limit(self):
        ...
        assert created["strategy_name"] == "melon"
        ...
        assert created["prior_yes_price_at_entry"] == 0.849
        assert created["yes_price_at_buy"] == 0.911
        assert created["stop_price_at_entry"] == 0.78
        assert created["take_profit_price_at_entry"] == 0.97
        assert created["entry_prob_min_at_buy"] == 0.85
        assert created["entry_prob_max_at_buy"] == 0.93
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (make_candidate(prior_yes_price=0.85), FakeClob(), FakeRepo()),
...
class TestStopExecution:
    def test_absolute_stop_uses_yes_signal_and_executes_at_fresh_bid(self):
        clob = FakeClob(midpoint=0.78, best_bid=0.77, best_ask=0.79)
        trader, repo, _ = make_trader(clob=clob)
        ...
        assert update["yes_price_at_exit"] == 0.78
        assert update["sell_price"] == 0.77
        assert update["best_bid_at_exit"] == 0.77
        assert update["best_ask_at_exit"] == 0.79
    ...
    def test_take_profit_waits_when_fresh_bid_is_below_target(self):
        clob = FakeClob(midpoint=0.965, best_bid=0.955, best_ask=0.965)
        trader, repo, _ = make_trader(clob=clob)
        assert trader.execute_sell(make_trade()) is False
        assert clob.orders == []
        assert repo.updates == []
    ...
    def test_partial_token_balance_never_creates_an_unmodelled_partial_sell(self):
        ...
        clob = FakeClob(
            midpoint=0.78, best_bid=0.77, best_ask=0.79, order_result=result
        )
        trader, repo, _ = make_trader(clob=clob)
        assert trader.execute_sell(make_trade()) is False
    ...
    def test_simulation_stop_is_hypothetical_not_realized(self):
        ...
        assert update["realized_pnl"] is None
        assert update["hypothetical_pnl"] == pytest.approx((0.77 - 0.915) * 5.2)
