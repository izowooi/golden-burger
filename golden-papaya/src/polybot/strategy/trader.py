"""Order execution and evidence-safe settlement handling for Final Five."""
...
class Trader:
    """Execute YES-only entries and absolute-stop exits."""
    ...
    def execute_buy(self, candidate: dict) -> Optional[int]:
        ...
        if candidate.get("outcome") != "Yes":
            logger.error("Final Five가 YES 이외 후보를 거부했습니다: %s", condition_id)
            return None
        ...
        buy_shares = self.config.buy_amount_usdc / best_ask
        ...
        logger.info(
            "Final Five 매수: '%s' signal=%.2f%% ask=%.2f%% shares=%.4f",
    ...
    def execute_sell(self, trade) -> bool:
        """Trigger only on YES midpoint <= stop; submit at the fresh best bid."""
        try:
        ...
        reason = evaluate_exit(current_yes, stop_price)
        if reason is None:
            logger.debug(
                "보유 유지: condition=%s YES=%.2f%% stop=%.2f%%",
                trade.condition_id,
                current_yes * 100,
                stop_price * 100,
            )
        ...
        if book is None:
            return False
        best_bid, best_ask, spread = book
        # Do not re-trigger from bid.  The stop was already decided by current_yes.
        logger.info(
            "absolute stop 충족: Trade #%s signal=%.2f%% bid=%.2f%% shares=%.6f",
            trade.id,
            current_yes * 100,
            best_bid * 100,
            trade.buy_shares,
        ...
        if result.get("success") or result.get("orderID"):
            ...
            if self.mode == "sim":
                ...
                logger.info(
                    "simulation stop SELL: Trade #%s bid=%.4f hypothetical P&L=$%.4f",
                    trade.id,
                    best_bid,
                    hypothetical_pnl,
                )
            ...
            logger.info(
                "stop SELL 접수, confirmed fill 대기: Trade #%s order=%s "
                "bid=%.4f size=%.6f",
                trade.id,
                result.get("orderID"),
                best_bid,
