#!/usr/bin/env python3
"""Offline, replayable research backtest for Golden Papaya.
...
and stops receive the observed bid.  These are explicitly hypothetical research
fills: they make no claim that a live GTC order was accepted or that an exact
``order_fills.status=CONFIRMED`` row exists.  Positions that do not stop are
valued only at the resolution payout; no synthetic take-profit or time exit exists.
"""
...
def replay_market(
    ...
    for current in observations[entry_index + 1 :]:
        # Stop signal is the YES reference price.  The bid is execution only;
        # using bid as the trigger would stop early merely because of spread.
    ...
    *,
    ...
    manifest = {
        ...
        "assumptions": {
            ...
            "stop": "absolute threshold; hypothetical fill at observed best_bid",
            "terminal": "authoritative resolution_payout; no TP/time exit",
            "execution_views": {
