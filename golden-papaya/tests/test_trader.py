"""Final Five order-book execution and settlement-evidence contracts."""
...
class TestEntryExecution:
    def test_buy_revalidates_crossing_and_executes_at_fresh_ask(self):
        clob = FakeClob(midpoint=0.951, best_bid=0.946, best_ask=0.956)
        ...
        assert clob.orders == [
            {
                "token_id": "yes-token",
                "price": 0.956,
                "size": pytest.approx(5.0 / 0.956),
                "side": "BUY",
            }
        ]
        created = repo.created[0]
        assert created["outcome"] == "Yes"
        assert created["strategy_name"] == "papaya"
        assert created["mode"] == "live"
        assert created["status"] == TradeStatus.PENDING_BUY
        assert created["event_id"] == "event-1"
        assert created["buy_price"] == 0.956
        assert created["buy_probability"] == 0.951
        assert created["prior_yes_price_at_entry"] == 0.949
        assert created["yes_price_at_buy"] == 0.951
    ...
    def test_minimum_share_buffer_is_enforced_at_actual_ask(self):
        config = TradingConfig(min_order_buffer_shares=0.20)
        clob = FakeClob(midpoint=0.95, best_bid=0.96, best_ask=0.97)
...
class TestStopExecution:
    def test_absolute_stop_uses_yes_signal_and_executes_at_fresh_bid(self):
        clob = FakeClob(midpoint=0.90, best_bid=0.885, best_ask=0.905)
        trader, repo, _ = make_trader(clob=clob)
        ...
        assert update["yes_price_at_exit"] == 0.90
        assert update["sell_price"] == 0.885
        assert update["best_bid_at_exit"] == 0.885
        assert update["best_ask_at_exit"] == 0.905
        assert update["spread_at_exit"] == pytest.approx(0.02)

    def test_low_bid_alone_does_not_trigger_stop(self):
        clob = FakeClob(midpoint=0.901, best_bid=0.85, best_ask=0.91)
        trader, repo, _ = make_trader(clob=clob)
        assert trader.execute_sell(make_trade()) is False
        assert clob.orders == []
        assert repo.updates == []

    @pytest.mark.parametrize("midpoint", [0.91, 0.97, 0.99])
    def test_no_take_profit_or_time_exit(self, midpoint):
    ...
    def test_invalid_or_unavailable_book_never_submits_stop(self):
        clob = FakeClob(midpoint=0.89, best_bid=None, best_ask=0.91)
        trader, repo, _ = make_trader(clob=clob)
        assert trader.execute_sell(make_trade()) is False
        assert clob.orders == []
        assert repo.updates == []

    def test_partial_token_balance_retries_once_with_safety_factor(self):
        ...
        assert repo.updates[-1][1]["sell_shares"] == pytest.approx(5.049)
...
class TestPendingBuyReconciliation:
    ...
    def test_expired_pending_buy_is_canceled_for_next_cycle_reconciliation(self):
        ...
        trade = make_trade(buy_timestamp=now - timedelta(minutes=30))
