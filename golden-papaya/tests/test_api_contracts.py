...
surface in the Papaya package rather than strategy decisions.
"""
...
def test_expired_entry_cancel_preserves_terminal_matched_quantity(status, matched):
    ...
    result = wrapper.cancel_order_for_reconciliation(
        "cancel-me", minimum_age_minutes=30
    )
...
def _placement_wrapper(db_path, client):
    wrapper = ClobClientWrapper(
        SimpleNamespace(), audit_db_path=db_path, strategy_name="golden-papaya"
    )
    wrapper._client = client
    wrapper._initialized = True
    return wrapper
