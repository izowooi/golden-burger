...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Polymarket Automated Trading Bot (Golden Fig - Hope Crusher)",
...
Examples:
  ...
  polybot config
    ...
    if args.command == "run":
        ...
        print()
        print("=== Hope Crusher Strategy ===")
        print(f"YES Band: {strategy.yes_min:.0%} ~ {strategy.yes_max:.0%} "
              f"(NO 매수 밴드: {1 - strategy.yes_max:.0%} ~ {1 - strategy.yes_min:.0%})")
        print(f"YES Rise Block (24h): > {strategy.yes_rise_block_24h:+.2f}")
        print(f"YES Spike Block (6h): >= {strategy.yes_spike_block_6h:.2f}")
        print(f"Entry Window: {time_based.entry_hours_min}h ~ {time_based.entry_hours_max}h")
