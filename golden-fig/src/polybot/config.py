...
def _validate_config(trading: TradingConfig, api: ApiConfig) -> None:
    ...
    numeric = {
        ...
        "strategy.yes_min": strategy.yes_min,
        "strategy.yes_max": strategy.yes_max,
        "strategy.yes_rise_block_24h": strategy.yes_rise_block_24h,
        "strategy.yes_spike_block_6h": strategy.yes_spike_block_6h,
        "strategy.rise_lookback_hours": strategy.rise_lookback_hours,
        "strategy.spike_lookback_hours": strategy.spike_lookback_hours,
        "time_based.entry_hours_min": timing.entry_hours_min,
        "time_based.entry_hours_max": timing.entry_hours_max,
    ...
    if not 0 < strategy.yes_min < strategy.yes_max < 1:
        raise ValueError("strategy YES bounds must satisfy 0 < yes_min < yes_max < 1")
    if not -1 <= strategy.yes_rise_block_24h <= 1:
        raise ValueError("strategy.yes_rise_block_24h must be between -1 and 1")
    if not 0 < strategy.yes_spike_block_6h < 1:
        raise ValueError("strategy.yes_spike_block_6h must be between 0 and 1")
    if not 0 < strategy.spike_lookback_hours < strategy.rise_lookback_hours:
        raise ValueError("strategy lookbacks must satisfy 0 < spike_lookback_hours < rise_lookback_hours")
    if not 0 < timing.entry_hours_min < timing.entry_hours_max:
        raise ValueError("time_based entry hours must satisfy 0 < entry_hours_min < entry_hours_max")
    if not 0 < timing.exit_hours <= timing.entry_hours_min:
        raise ValueError("time_based.exit_hours must be > 0 and <= entry_hours_min")
