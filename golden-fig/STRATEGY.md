...
## ⛔ 이 가설은 기각되었다 (2026-07-28)

- **필요 승률 69.0% vs 실제 57.5% — 11.5pp 부족.** 설정 손절 -10%가 실제로는 **-18.96%**로 실현되어(갭 통과) 손익비가 1:2.23으로 벌어졌다.

- **캘리브레이션 5개 가격 구간이 전부 edge 음수.** 거래 시장 1,567건의 실제 해결 결과를 대조한 결과 진입가 0.8489 vs 실현률 83.41%로 **-1.48pp**. 5/5 구간 동일 방향은 부호검정 p=3.1%. **favorite-longshot bias는 이 데이터에 존재하지 않는다.** 종결 ROI -1.9%는 *모든 포지션을 해결까지 들고 갔어도 손실*이라는 뜻이다.

- **전략이 자기 우주에서 실행되지 않았다.** `excluded_categories: []`로 스포츠 필터가 꺼져 있어 outcome 라벨이 **698종**(테니스 핸디캡, 축구 프롭, 선수 이름…)이었다. theta 수확 논리가 성립하지 않는 시장들이다. **그런데 본래 대상인 `outcome='No'`만 봐도 -$125.10 손실**이라 필터를 켜도 살아나지 않는다.

- **장부가 명목을 약 4배 부풀린다.** BUY 제출 4,501건 중 전량체결은 **23.4%**뿐(순수 유령 57.9%). 독립 교차검증: HOLDING 653건×$5 = 장부 $3,265 vs 지갑 실제 $794.60 = 24.3%.

- **리스크 통제 부재.** `max_positions: -1`(무제한), 지갑 잔고 사전 확인 없음, 이벤트 단위 상한 없음 → Position $794.60 / Cash $3.44. 매도 제출 13,395건 중 **11,777건(87.9%) 실패**(부분체결 무한 재시도).

- 전략 자체 중단 기준(`STRATEGY.md:95`: stop_loss 25% 초과)도 이미 발동했다 — **실제 33.1%**.

**파라미터로 기대값의 부호를 바꿀 수 있는 조합이 없다.**

**→ 상세: [docs/retro/golden-mango-fig-2026-07-verdict.md](../docs/retro/golden-mango-fig-2026-07-verdict.md)**

아래 내용은 **폐쇄 판정 이전에 작성된 원래 설계 문서**다. 역사 자료로만 읽을 것.
