...
**수익원은 진입 신호가 아니라 실행 측면이다.** 처치는 **BUY 전용**이며 실측 진입 비용은
MAKER `-16.8 bps` / TAKER `+56.7 bps`로 약 73 bps 갈린다. `execution_mode`
(passive/nearest/cross)가 그 축이다.

진입 신호는 golden-queen의 Crown Momentum(첫 0.90 상향 교차, 0.90–0.94 진입,
0.98 익절 / 0.85 손절)을 **의도적으로 그대로 상속**한다 — 신호가 아니라 실행이
처치이므로 A/B/C 전 팔에서 24시간 horizon을 포함한 신호가 동일해야 한다.

**A/B/C 3팔이 사전 등록돼 있다**: A=passive(처치), B=nearest(대조, 기존 14봇과 동일),
...
CLI `--live`로 명시적으로 해제할 때만 허용한다. simulation/live와 A/B/C variant는 서로
다른 `job_name`과 DB를 사용한다.

## Quince 고유 계약

- `execution_mode`는 **처치축**이며 **진입(BUY)에만 적용**된다. 30일 A/B/C 중에는 절대
...
- **낙폭 kill switch가 코드에 있다.** **경제손익(확정 `realized_pnl` + 해결
  `settlement_pnl_assumption`)** 이 `experiment_capital_usdc x max_drawdown_stop`(기본 $40) 이하가 되면
...
- `get_stats()`의 확정손익과 해결 추정은 성과 보고에서 합산하지 않고 kill switch에서만 합산한다.
...
- 유효한 직전 persisted YES가 0.90 미만이고 현재 YES가 `[0.90, 0.94]`인 Quince 자체
  archive의 **first observed upward crossing**만 인정한다. 현재 snapshot은 이번 sweep에서 commit된 양의 ID여야 하며 직전 snapshot과
  `0 < gap <= 15분`이어야 한다.
...
- resolution 결과, redeemable 상태, 실제 redeem transaction, SELL fill은 서로 다른
  lifecycle evidence다. 현재 Quince는 resolution 결과까지만 적재하고 실제 redeem
...
- A=`polybot-quince-passive`, B=`polybot-quince-nearest`, C=`polybot-quince-cross`다.
  Jenkins job과 `--job`을 동일한 이름으로 고정하고 각 `data/<job>/trades.db`를 분리한다.
- 세 팔은 서로 다른 wallet/account/funder/credential을 쓰고 cadence는 모두
  `H/5 * * * *`다. 같은 wallet, job 또는 DB를 공유하면 live A/B/C 격리가 아니다.
- 세 팔 모두 주문액 $5, `POLYBOT_ENTRY_HOURS_MAX=24`, archive 72시간, threshold, risk,
  sports/in-play, lifecycle, Git commit을 동일하게 유지한다. 다른 것은 BUY
  `POLYBOT_EXECUTION_MODE`뿐이며 SELL은 언제나 `nearest`다.
- 선택적 D는 `passive` + $10만 바꾸고 별도 wallet/job/DB cohort로 둔다. A/B/C가 우선이다.
...
- 30일 1차 종점은 MAKER 비중 `A > B > C`, 진입 실효가−결정 midpoint
  `A < B < C`, 체결률 `A < B < C`다. A의 CONFIRMED BUY가 30건 미만이면 판정
  불가이며, 승률·순손익은 1차 판정에 쓰지 않는다.

## 변경 시 동기화와 금지사항

- 전략 수치를 바꾸면 `config.yaml`, `src/polybot/config.py`, signal/timing tests,
  `STRATEGY.md`, `README.md`, retro의 30일 execution endpoint를 함께 갱신한다.
