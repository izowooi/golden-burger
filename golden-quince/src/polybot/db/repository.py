...
class ExactFillEvidence:
    ...
    def has_reconciled_executed_fill(self) -> bool:
        """Whether the terminal order's entire executed quantity is proven.

        A canceled passive order can be partially filled.  It is not a full
        *requested-order* fill, but once the terminal matched size equals the
        sum of exact CONFIRMED fills it is a real position that must enter exit
        management instead of remaining in ``PENDING_BUY`` forever.
        """
        return (
...
class TradeRepository:
        ...
        *,
        ...
        if confirmed:
            ...
            reconciled_full_fill = (
                reconciled_executed_fill
                and (
                    # MATCHED is the venue's terminal full-order state.  Its
                    # matched size is quantized and can be slightly below the
                    # pre-quantization intent.
    ...
    def cleanup_old_snapshots(self, days: int = 60) -> int:
        if compact_maintenance_active(self.session, "golden-quince"):
            return 0
        ...
        self.session.execute(
            text(
                ...
                "_polybot_quince_protected_snapshots "
                "(id INTEGER PRIMARY KEY) WITHOUT ROWID"
            )
        )
        self.session.execute(text("DELETE FROM _polybot_quince_protected_snapshots"))
        self.session.execute(
            text(
                "INSERT OR IGNORE INTO _polybot_quince_protected_snapshots(id) "
        ...
        deleted = self.session.execute(
            text(
                ...
                "AND id NOT IN ("
                "SELECT id FROM _polybot_quince_protected_snapshots)"
            ),
            {"cutoff": cutoff},
        ).rowcount
        self.session.execute(text("DROP TABLE _polybot_quince_protected_snapshots"))
        expired_sweeps = [
            row[0]
