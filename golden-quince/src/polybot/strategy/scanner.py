"""Gamma universe archive and threshold-crossing scanner for Crown Momentum."""
...
_QUINCE_ARCHIVE_MIN_LIQUIDITY = 1_000.0
_QUINCE_ARCHIVE_MIN_CUMULATIVE_VOLUME = 1_000.0
...
class MarketScanner:
    ...
    def __init__(
        ...
        self.config = config
        self.repo = repo
        # Kept in the signature for shared construction code; Crown Momentum relies
    ...
    def fetch_markets(self) -> List[Dict]:
        ...
        archive_min_liquidity = min(
            self.config.min_liquidity, _QUINCE_ARCHIVE_MIN_LIQUIDITY
        )
        ...
        return self.gamma.get_all_tradable_markets(
            ...
            min_volume=_QUINCE_ARCHIVE_MIN_CUMULATIVE_VOLUME,
        )
    ...
    def save_market_snapshots(
        ...
        if self.repo is None:
            raise RuntimeError("repository is required for Crown Momentum archive evidence")
        ...
        try:
            ...
            raise
        logger.info(
            "Crown Momentum research snapshot %s개 저장 (strict binary, YES>=%.2f, <=%.0fh)",
            saved,
    ...
    def _entry_snapshot_lineage(
        ...
        # Sixty-day retention is much wider than Queen's three-day archive.
    ...
    def scan_buy_candidates(
        ...
        """Return markets that have just crossed into the Crown Momentum band."""
        reference = now or datetime.now(timezone.utc)
        if reference.tzinfo is None:
            reference = reference.replace(tzinfo=timezone.utc)
        ...
        logger.info("Crown Momentum 매수 후보 %s개 발견", len(candidates))
        return candidates
