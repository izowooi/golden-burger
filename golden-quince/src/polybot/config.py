...
def _get_execution_mode(yaml_value) -> str:
    """Resolve the tick-rounding side. env > YAML > "passive".

    이 값이 golden-quince의 실험 처치다. 실측 왕복 비용이
    maker→maker -31.1 bps / taker→taker +72.5 bps 로 **103 bps** 갈리는데,
    기존 14개 봇은 side를 보지 않는 `round()` 때문에 이 축이 무작위였다.
    """
...
class TradingConfig:
    ...
    experiment_capital_usdc: float = 200.0
