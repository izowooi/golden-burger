...
Golden Quince은 `golden-cherry`의 “해결이 가까운 우세 YES가 수렴한다”는 뼈대를 계승하면서,
...
uv run polybot run --live --job quince-passive
```
...
| 값 | BUY | SELL | 역할 |
|---|---|---|---|
| `passive` (기본) | 내림 | 올림 | 절대 크로스하지 않는다 — 처치군 |
| `nearest` | 반올림 | 반올림 | 기존 14개 봇과 동일 — 대조군 |
| `cross` | 올림 | 내림 | 항상 크로스 — 비용 상한 |

실측 왕복 비용이 maker→maker `-31.1 bps`, taker→taker `+72.5 bps`로 **103 bps**
갈린다. 근거와 A/B 판정 기준은 `STRATEGY.md` §2·§6.
...
join해야 한다. 월간 절차는 [golden-quince 회고 문서](../docs/retro/golden-quince.md)를 따른다.
