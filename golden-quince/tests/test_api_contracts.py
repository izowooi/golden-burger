...
def test_expired_entry_cancel_uses_exact_catalog_zero_fill_fallback():
    captured = {}

    class MissingDetailClient:
        def cancel_orders(self, order_ids):
            assert order_ids == ["cancel-me"]
            return {"canceled": ["cancel-me"], "not_canceled": {}}
        ...
        def get_trades(self, params, *, only_first_page):
            assert str(params.asset_id) == "token"
            assert only_first_page is False
            return []

    class Ledger:
        def pending_submissions(self):
            return [{"order_id": "cancel-me", "token_id": "token"}]

        def record_expired_gtc_zero_fill(self, **evidence):
            captured.update(evidence)
            return "EXACT_GTC_CANCEL_ACK_ZERO_FILL"

    wrapper = ClobClientWrapper(SimpleNamespace())
    wrapper._client = MissingDetailClient()
    wrapper._initialized = True
    ...
    assert captured["minimum_age_minutes"] == 15
...
def _placement_wrapper(db_path, client):
    wrapper = ClobClientWrapper(
        SimpleNamespace(), audit_db_path=db_path, strategy_name="golden-quince"
    )
    wrapper._client = client
    wrapper._initialized = True
    return wrapper
