...
def test_crown_cross_defaults_are_fail_closed():
    ...
    # Spread Harvest 고유 계약: 기본은 절대 크로스하지 않는 패시브 실행이고,
    # 낙폭 kill switch가 코드에 존재한다.
    assert trading.execution_mode == "passive"
    assert trading.intent_autoresolve is True
    assert trading.max_drawdown_stop == 0.20
    assert trading.experiment_capital_usdc == 200.0
