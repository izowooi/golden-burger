...
실거래의 cycle SELL 제출 한도는 새 주문 제출만 막고, 뒤 포지션의 호가·해결 대사는
계속한다. POST 결과 불명확 또는 제출 원장 오류도 한도에 예약하며, 불명확 SELL은
PENDING_SELL 또는 격리 상태에 남겨 확인 전 재제출하지 않는다.

Grey는 계좌가 없는 연구 수집기이므로 가상 손실·전체 가상 포지션 수·cycle 진입 수·가상
SELL 수의 실거래 한도로 이후 경기 표본을 자르지 않는다. 실제 계좌의 동일 한도는 계속
강제한다. Grey에도 event당 한 번, source/book 증거, 기간과 실행시간 제한은 유지한다.
