...
BUY 상태 불명확과 fill/fee evidence gap은 중복 노출 위험 때문에 신규 진입을 전체 중지한다.
SELL-only 실패는 해당 event에만 국소화한다. 한 SELL이 거절·미체결되어도 다른 holding의
관리와 다른 event 진입은 계속한다. 연속 실패가 180분을 넘으면 `QUARANTINED`로 옮기되,
성공 매도나 실현 손익을 만들지 않고 위험 한도를 계속 예약한다. 뒤늦은 exact fill/zero-fill 또는
resolution 증거로만 상태를 바꾼다.
