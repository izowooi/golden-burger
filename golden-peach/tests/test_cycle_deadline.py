...
def test_live_budget_never_suppresses_reconciliation_requests_or_post() -> None:
    clock = FakeClock()
    budget = CycleBudget.start(enforce_deadline=False, monotonic=clock)
    clock.value = 75.0

    budget.ensure_can_start_request("live reconciliation")
    budget.assert_within_hard_deadline("live POST boundary")
...
def test_context_manager_enforces_completion_boundary() -> None:
    ...
    with pytest.raises(CycleDeadlineExceeded, match="cycle completion"):
        with enforced_cycle_deadline(enforce_deadline=True, monotonic=clock):
            clock.value = 50.0


def test_context_manager_keeps_live_warning_only() -> None:
    clock = FakeClock()
