...
매수하는 live A/B + shadow 전략이다. 현재 Eco/Fruit live는 축구 `$5`로 고정되어 있고,
Grey는 축구와 향후 MLB/NBA/NFL/NHL 확대에 필요한 종목별·거래규모별 증거를 저장할 수 있다.
...
세 job은 1분 cadence, T7 외장 workspace를 사용한다. 현재 live A/B의 유일한 처리축은 TP
폭이며, 주문 목표액이나 종목을 바꾼 구간은 별도 cohort로 평가한다.
...
- direct team 종목은 현재 Grey shadow만 허용한다. 예정 시작 시각 경과값은 수집 보조값일 뿐
  live 진입 clock으로 인정하지 않으며, 종목별 TP/SL 근거가 쌓이기 전 live 실행은 거부한다.
...
사전 등록과 다르거나 direct team 종목을 live로 요청하면 network와 DB mutation 전에 실패한다.
