# Golden Peach 사전 등록 — kickoff-leader-v2
...
- 명확한 주문 전 오류와 동기식 0주문 거절은 해당 후보만 실패시키며 다음 event를
  계속 처리한다.
- 불명확한 BUY/SELL은 해당 event에만 격리하고 실제일 수 있는 노출 1칸을
  `max_positions=10`에 예약한다. 남은 예약 한도에서는 다른 event를 계속 처리한다.
- BUY/SELL pending 또는 연속 청산 실패가 180분을 넘으면 성공 체결이나 미체결로
  꾸미지 않고 `QUARANTINED`로 전환한다. 같은 event 재진입은 계속 금지하고 exact
  fill/zero-fill 또는 resolution 증거가 생길 때까지 한도 1칸을 예약한다.
- 실행 원장이 주문 전후를 구별할 수 없을 정도로 손상되었거나 실제 예약 노출이
  10칸이면 신규 BUY를 중지한다.
