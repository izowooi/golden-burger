...
class TradeRepository:
    ...
    def get_latest_snapshot(
        ...
        """마켓의 최신 스냅샷 조회 (retrace_target 청산 판정용 YES 가격)."""
        return self.session.query(MarketSnapshot).filter(
            MarketSnapshot.condition_id == condition_id
        ).order_by(
    ...
    def cleanup_old_snapshots(self, days: int = 21) -> int:
        """오래된 스냅샷 정리 (디스크 공간 관리).

        Args:
            days: 보관 일수 (기본 21일 - 전략 lookback 7d의 3배)

        Returns:
            삭제된 스냅샷 수
        """
        if compact_maintenance_active(self.session, "golden-orange"):
            return 0
    ...
    def append_trade_to_csv(self, trade: Trade, db_dir) -> None:
        """완료된 거래를 월별 CSV 파일에 추가 (시그널 컬럼 포함 - 부록 §A.7).
        ...
        row = {
            ...
            "buy_price": _num(trade.buy_price),
            "sell_price": _num(trade.sell_price),
            ...
            "hours_until_resolution_at_buy": _num(trade.hours_until_resolution_at_buy),
            "buy_probability": _num(trade.buy_probability),
            "sell_probability": _num(trade.sell_probability),
            "volume_24h_at_buy": _num(trade.volume_24h_at_buy),
            "yes_price_at_buy": _num(trade.yes_price_at_buy),
            "base_price_at_buy": _num(trade.base_price_at_buy),
            "spike_peak_at_buy": _num(trade.spike_peak_at_buy),
            "spike_age_minutes_at_buy": _num(trade.spike_age_minutes_at_buy),
            "vol_mult_at_buy": _num(trade.vol_mult_at_buy),
            "yes_price_at_exit": _num(trade.yes_price_at_exit),
