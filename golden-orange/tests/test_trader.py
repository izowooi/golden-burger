"""Trader 방어 로직 테스트 (midpoint 0 투매 방지 + EXPIRED 마감 + §A 컬럼 기록 + UNFILLED 유령 포지션)."""
...
class TestUnfilledPhantomDetection:
    def _phantom_trader(self):
        repo = FakeRepo()  # 스냅샷 없음 → retrace 보류
        clob = PhantomClob(midpoint=0.85)  # 0.78 -> 0.85 = +9.0% -> take_profit 발동
