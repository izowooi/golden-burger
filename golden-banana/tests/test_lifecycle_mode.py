"""수명주기 모드가 Banana의 snapshot과 주문 경로를 올바르게 분리하는지 검증."""
...
def _build_bot(monkeypatch, tmp_path, lifecycle_mode: str, holdings):
    markets = [{"condition_id": "market-1"}]
    scanner = MagicMock()
    scanner.fetch_markets.return_value = markets
    ...
    return bot, scanner, trader, repo, session, markets


def test_close_only_keeps_snapshots_and_exits_but_blocks_entries(monkeypatch, tmp_path):
    trade = SimpleNamespace(id=1, token_id="token-1")
    bot, scanner, trader, repo, session, markets = _build_bot(
    ...
    scanner.save_market_snapshots.assert_called_once_with(markets)
    scanner.scan_buy_candidates.assert_not_called()
    trader.execute_buy.assert_not_called()
    repo.cleanup_old_snapshots.assert_called_once_with(days=7)
    session.close.assert_called_once()


def test_archive_only_keeps_snapshots_without_touching_orders(monkeypatch, tmp_path):
    bot, scanner, trader, repo, session, markets = _build_bot(
...
def test_active_keeps_existing_entry_path(monkeypatch, tmp_path):
    ...
    scanner.scan_buy_candidates.assert_called_once_with(markets)
